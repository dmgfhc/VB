...
Begin VB.Form CGD2041C 
   BackColor       =   &H00E0E0E0&
   Caption         =   "钢板取样信息查询及修改界面_CGD2041C"
   ClientHeight    =   8775
   ClientLeft      =   360
   ClientTop       =   1890
   ClientWidth     =   14340
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.ComboBox CBO_GROUP 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "CGD2041C.frx":0000
         Left            =   3990
         List            =   "CGD2041C.frx":0010
         TabIndex        =   18
         Top             =   600
         Width           =   705
      End
      ...
      Begin VB.ComboBox CBO_SHIFT 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "CGD2041C.frx":0020
         Left            =   3990
         List            =   "CGD2041C.frx":002D
         TabIndex        =   9
         Top             =   210
         Width           =   705
      End
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin FPSpread.vaSpread ss1 
         ...
         SpreadDesigner  =   "CGD2041C.frx":008C
      End
      Begin FPSpread.vaSpread ss2 
         ...
         Protect         =   0   'False
         RetainSelBlock  =   0   'False
         SpreadDesigner  =   "CGD2041C.frx":0930
      End
   End
   Begin VB.Line Line1 
      X1              =   60
...
Const SS1_STLGRD = 3                    '钢种

Const SS2_PLATE_NO = 2                  'PLATE NO
Const SS2_PROC_CD = 7                   '进程代码
Const SS2_PROD_CD = 3                   'PRODUCT STATUS
Const SS2_SMP_FLAG = 17                 '实绩标记
Const SS2_SMP_LOC = 18                  '位置
Const SS2_SMP_LEN = 19                  '长度
Const SS2_SMP_NO = 20                   '试样号
Const SS2_STDSPEC = 21                  '标准号
Const SS2_BEF_STDSPEC = 22              'BEFORE 标准号
Const SS2_USER_ID = 29                  'USER ID
Const SS2_BEF_SMP_FLAG = 30             'BEFORE 实绩标记
Const SS2_BEF_SMP_LOC = 31              'BEFORE 位置
Const SS2_BEF_SMP_LEN = 32              'BEFORE 长度
Const SS2_BEF_SMP_NO = 33               'BEFORE 试样号
Const SS2_CHG_SMP_NO = 34               '改判时试样号
Const SS2_CHG_STDSPEC = 35              '改判时适用标准
...
Private Sub Form_Define()
    ...
    sc1.Add Item:="CGD2041C.P_REFER1", Key:="P-R"
    ...
    Call Gp_Sp_Collection(ss2, 34, " ", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)
    Call Gp_Sp_Collection(ss2, 35, " ", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)
    Call Gp_Sp_Collection(ss2, 36, " ", " ", " ", " ", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)

    'Spread_Collection
    sc2.Add Item:=ss2, Key:="Spread"
    sc2.Add Item:="CGD2041C.P_MODIFY", Key:="P-M"
    sc2.Add Item:="CGD2041C.P_REFER2", Key:="P-R"
...
Private Sub Cmd_Set_Save_Click()
    ...
    strQuery = strQuery & "         ,MARK_DATA10                                                " & vbCrLf
    ...
    strQuery = strQuery & "         ,INSP_T_FLAW                                                " & vbCrLf
    strQuery = strQuery & "         ,INSP_B_FLAW                                                " & vbCrLf
    strQuery = strQuery & "         ,INSP_B_DEP_FLAW2                                           " & vbCrLf
    strQuery = strQuery & "         ,INSP_T_DEP_FLAW2                                           " & vbCrLf
    ...
    strQuery = strQuery & "         ,(SELECT N.MARK_DATA10 FROM EP_PLATE_INS3 N WHERE N.SLAB_NO = A.SLAB_NO AND ROWNUM =1)  MARK_DATA10          " & vbCrLf
    strQuery = strQuery & "         ,A.TRNS_CMPY_CD                            MOCUT_NO         " & vbCrLf
    ...
    strQuery = strQuery & "         ,GF_COMNNAMEFIND('G0002',I.INSP_T_FLAW)    INSP_T_FLAW      " & vbCrLf
    strQuery = strQuery & "         ,GF_COMNNAMEFIND('G0002',I.INSP_T_FLAW)    INSP_B_FLAW      " & vbCrLf
    strQuery = strQuery & "         ,GF_COMNNAMEFIND('G0002',I.INSP_T_FLAW)    INSP_B_DEP_FLAW2 " & vbCrLf
    strQuery = strQuery & "         ,GF_COMNNAMEFIND('G0002',I.INSP_T_FLAW)    INSP_T_DEP_FLAW2 " & vbCrLf
    ...
    strQuery = strQuery & "   FROM  GP_PLATE A,GP_INSPPLATE  I,BP_ORDER_ITEM  B                 " & vbCrLf
    strQuery = strQuery & "   WHERE (A.REC_STS          =     '2')     AND                      " & vbCrLf
    strQuery = strQuery & "         (A.PRC_LINE        <=     '2')     AND                      " & vbCrLf
    ...
    strQuery = strQuery & "         A.PLATE_NO          =     I.MAT_NO(+)    AND                " & vbCrLf
...
'            End If
            ...
            If sSpec <> ss1.Text Then
                Call Gp_MsgBoxDisplay("钢种不一致，请确认")
                Exit Sub
            End If
            iSelCnt = iSelCnt + 1
        End If
