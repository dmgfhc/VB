...
Private Sub Form_Load()
    ...
    If FORM_A = "ACB1020C" Or FORM_A = "ACB4110C" Then
...
Public Sub Form_Ref()
    ...
    Call SELECT_PRC
    sQuery = "select to_char(TO_DATE(t.ins_date,'YYYYMMDDHH24MISS'),'YYYY-MM-DD') from gp_ustresult t WHERE t.plate_no= '" + Trim(txt_mat_no.Text) + "'"
    txt_ust_date.Text = Gf_CodeFind(M_CN1, sQuery)
    
    sQuery = "select t.ust_grd from gp_ustresult t WHERE t.plate_no= '" + Trim(txt_mat_no.Text) + "'"
    txt_ust_GRD.Text = Gf_CodeFind(M_CN1, sQuery)
    
    sQuery = "SELECT IN_SLAB_NO FROM FP_SLAB WHERE SLAB_NO = '" + Trim(txt_mat_no.Text) + "'"
    txt_in_slab_no.Text = Gf_CodeFind(M_CN1, sQuery)
...
Private Sub SELECT_PRC()
    ...
    If WULIAO = "SL" Then
        ...
        S1.Visible = True
        ULabel57.Caption = "入库日期"
        ULabel58.Caption = "入库时间"
    ElseIf WULIAO = "HC" Then
        sQuery1 = " SELECT * FROM GP_COIL WHERE COIL_NO    =  '" + select_no + "'"
        S2.Visible = True
        ULabel57.Caption = "终判日期"
        ULabel58.Caption = "终判时间"
    ElseIf WULIAO = "PP" Then
...
'        txt_IN_SHEET_NO.Text = Trim(AdoRs.Fields(49) & "")
        ...
        If WULIAO = "SL" Then
            ...
            txt_cust_spec_no.Text = Trim(AdoRs.Fields(108) & "")
        ElseIf WULIAO = "HC" Then
            ...
            txt_cust_spec_no.Text = Trim(AdoRs.Fields(107) & "")
        ...
        ElseIf WULIAO = "PP" Then
            ...
            Text_size_knd.Text = Trim(AdoRs.Fields(102) & "")
            txt_cust_spec_no.Text = Trim(AdoRs.Fields(105) & "")
            txt_shot_blast.Text = IIf(IsNull(Trim(AdoRs.Fields(118))), "NN", Trim(AdoRs.Fields(118))) & "/" & _
                                  IIf(IsNull(Trim(AdoRs.Fields(119))), "N", Trim(AdoRs.Fields(119)))
            txt_htm_meth.Text = Trim(AdoRs.Fields(121)) & "/" & Trim(AdoRs.Fields(123)) & " " & _
                               Trim(AdoRs.Fields(124)) & "/" & Trim(AdoRs.Fields(126)) & " " & _
                               Trim(AdoRs.Fields(127)) & "/" & Trim(AdoRs.Fields(129))
        ...
        Call CONVERT_C(Left(WULIAO, 1), Trim(txt_APLY_ENDUSE_CD))  '43
    ...
    sQuery1 = sQuery1 + "   AND SND_TIME = (SELECT MAX(SND_TIME) FROM CP_MOVE_SLT WHERE MAT_NO   = '" + Trim(txt_mat_no) + "') "
...
Private Sub EDIT_MV_LST_NO()
    Dim SQL As String
    Dim AdoRs As ADODB.Recordset
    Set AdoRs = New ADODB.Recordset

    SQL = "SELECT MV_LST_NO FROM CP_MOVE_SLT WHERE MAT_NO = '" & Trim(txt_no.Text) & "'"
    SQL = SQL & "  AND SND_TIME = ( SELECT MAX(SND_TIME) FROM CP_MOVE_SLT WHERE MAT_NO = '" & Trim(txt_no.Text) & "')"
    AdoRs.Open SQL, M_CN1, adOpenKeyset
    
    If AdoRs.BOF Or AdoRs.EOF Then
        Exit Sub
    End If
    
    txt_IN_SHEET_NO.Text = Trim(AdoRs.Fields(0) & "")
    AdoRs.Close
End Sub
...
Private Sub txt_no_Change()
    ...
    If Len(txt_no.Text) = 10 Then
        WULIAO = "SL"
    ElseIf Len(txt_no.Text) = 12 And Right(txt_no.Text, 2) = "00" Then
        WULIAO = "HC"
    ElseIf Len(txt_no.Text) = 14 Or Len(txt_no.Text) = 12 Then
        WULIAO = "PP"
    Else
        WULIAO = ""
        Exit Sub
