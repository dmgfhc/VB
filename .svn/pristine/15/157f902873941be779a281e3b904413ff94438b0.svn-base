...
Begin VB.Form ABX1050C 
   ...
   Begin Threed.SSPanel pnl_condition 
      ...
      Begin FPSpread.vaSpread ssWhere 
         ...
         ScrollBarExtMode=   -1  'True
         ScrollBars      =   1
         SpreadDesigner  =   "ABX1050C.frx":024A
         UserResize      =   1
      End
   End
...
End With
   ...
   sQuery = sQuery + " AND A.PROD_KND = SUBSTR( '" & Trim(ABA1020C.txt_prod_cd.Text) & "', 1, 1)   "
   sQuery = sQuery + " AND  A.STDSPEC  = '" & Trim(ABA1020C.txt_stdspec.Text) & "' AND  A.THK_MIN < = " & Trim(ABA1020C.sdb_ord_thk.Value)
   sQuery = sQuery + " AND  A.THK_MAX  > = " & Trim(ABA1020C.sdb_ord_thk.Value)
