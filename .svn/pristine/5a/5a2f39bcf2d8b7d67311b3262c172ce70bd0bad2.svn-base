...
Begin VB.Form AFO2060C 
   Caption         =   "炼钢原料出/入库(日)查询界面_AFO2060C"
   ClientHeight    =   9225
   ClientLeft      =   300
   ClientTop       =   1185
   ClientWidth     =   15225
   ...
   Begin Threed.SSCommand cmd_LST_D_MAT_CONF 
      Height          =   360
      Left            =   14130
      TabIndex        =   3
      Top             =   90
   ...
   Begin VB.ComboBox cbo_MAT_KIND 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "AFO2060C.frx":0000
      Left            =   5370
      List            =   "AFO2060C.frx":0002
      Style           =   2  'Dropdown List
      TabIndex        =   0
      Top             =   120
      Width           =   1560
   End
   Begin VB.TextBox txt_cfm 
      Height          =   270
      Left            =   9315
      TabIndex        =   2
      Text            =   "1"
   ...
   Begin FPSpread.vaSpread ss1 
      ...
      SpreadDesigner  =   "AFO2060C.frx":0004
   End
...
Private Sub Form_Define()
   ...
   Call Gp_Ms_Collection(cbo_MAT_KIND, "p", " ", " ", " ", "r", "", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
    Call Gp_Ms_Collection(txt_MAT_DAY, "p", "n", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
        Call Gp_Ms_Collection(txt_cfm, "p", " ", " ", " ", " ", "a", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
...
'---------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   Call Gp_Sp_Collection(ss1, 10, " ", " ", " ", "i", "a", " ", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1)
    
    'Spread_Collection
    sc1.Add Item:=ss1, Key:="Spread"
    sc1.Add Item:="AFO2060C.P_SMODIFY", Key:="P-M"
    sc1.Add Item:="AFO2060C.P_SREFER", Key:="P-R"
    sc1.Add Item:="AFO2060C.P_SONEROW", Key:="P-O"
...
Private Sub cmd_LST_D_MAT_CONF_Click()
       
    If Gf_MessConfirm("您确定要最终核实该天的库存量吗？", "Q", "") Then
        txt_cfm.Text = "1"
...
Private Sub Form_Load()
    ...
    cbo_MAT_KIND.AddItem "0:废钢"
    cbo_MAT_KIND.AddItem "1:辅原料"
    cbo_MAT_KIND.AddItem "2:铁合金"
    cbo_MAT_KIND.AddItem "3:其他原料"
    ...
    If Mid(sAuthority, 3, 1) <> "1" Then
       cmd_LST_D_MAT_CONF.Enabled = False
    Else
       cmd_LST_D_MAT_CONF.Enabled = True
    End If
...
'    If Gf_Sp_ProceExist(Proc_Sc("Sc").Item("Spread")) Then Exit Sub
    ...
    If Gf_Sp_Refer(M_CN1, Proc_Sc("Sc"), Mc1, Mc1("nControl"), Mc1("mControl")) Then
        ...
        MDIMain.MenuTool.Buttons(7).Enabled = False
        pControl(2).Enabled = False
        pControl(1).Enabled = False
               ...
               For j = 1 To .MaxRows
                  ...
                  If .Col = 7 Then
                      ...
                      Else
                          A = A + Val(.Text)
                      End If
                  ElseIf .Col = 5 Or .Col = 6 Or .Col = 8 Then
                     A = A + Val(.Text)
                  End If
               Next
