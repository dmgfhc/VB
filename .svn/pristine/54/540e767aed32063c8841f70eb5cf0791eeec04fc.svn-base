...
Begin VB.Form ACB5036C 
   Caption         =   "库内不同区域卸车实绩录入_ACB5036C"
   ClientHeight    =   9420
   ClientLeft      =   705
   ClientTop       =   2070
   ClientWidth     =   15255
   ...
   Begin SSSplitter.SSSplitter SSSplitter2 
      ...
      Begin Threed.SSFrame SSFrame1 
         ...
         Begin VB.CheckBox chk_Excel_Fl 
            ...
            Width           =   1815
         End
         Begin VB.TextBox text_cur_area_name 
            Enabled         =   0   'False
            BeginProperty Font 
               Name            =   "宋体"
               Size            =   9.75
            ...
            Width           =   1230
         End
         Begin VB.TextBox txt_to_area_name 
            Enabled         =   0   'False
            BeginProperty Font 
               Name            =   "宋体"
               Size            =   9.75
         ...
         Begin VB.TextBox txt_mv_lst_no 
            ...
            Width           =   1830
         End
         Begin VB.TextBox text_cur_area_code 
            Alignment       =   2  'Center
            BeginProperty Font 
               Name            =   "宋体"
               Size            =   9.75
      ...
      Begin SSSplitter.SSSplitter SSSplitter1 
         ...
         Begin FPSpread.vaSpread ss2 
            ...
            SpreadDesigner  =   "ACB5036C.frx":00A4
         End
         Begin FPSpread.vaSpread ss1 
            ...
            ScrollBarExtMode=   -1  'True
            SpreadDesigner  =   "ACB5036C.frx":0838
         End
      End
   End
End
Attribute VB_Name = "ACB5036C"
...
Const SS1_ORD_REMARK = 38
Const SS1_STDSPEC_ORG_KND = 39
Const SS1_STDSPEC_STLGRD = 40
Const SS1_MV_DATE = 41
Const SS1_Shift = 42
Const SS1_TRNS_CMPY_CD = 43
Const SS1_CUST_CD1 = 44

Const SS1_PLATE_CON = 52   '子板数
Const SS1_PLATE_SIZE = 53   '子板尺寸
...
Private Sub Form_Define()
     ...
     'Form Type : Start , Master, Sheet, Msheet, PopSheet, Refer
      FormType = "Msheet"
           ...
           Call Gp_Ms_Collection(text_prod_cd, "p", "n", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
            Call Gp_Ms_Collection(text_cur_inv, "p", "n", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
      Call Gp_Ms_Collection(text_cur_area_code, "p", "n", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
             Call Gp_Ms_Collection(txt_to_area, "p", "n", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
   ...
   Call Gp_Sp_Collection(ss1, 54, " ", " ", " ", "i", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1)
  
    'Spread_Collection
    sc1.Add Item:=ss1, Key:="Spread"
    sc1.Add Item:="ACB5036C.P_REFER", Key:="P-R"
    sc1.Add Item:="ACB5036C.P_ONEROW", Key:="P-O"
    sc1.Add Item:="ACB5036C.P_MODIFY", Key:="P-M"
   ...
   Call Gp_Sp_Collection(ss2, 16, " ", " ", " ", " ", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)
    ...
    sc2.Add Item:="ACB5036C.P_SREFER", Key:="P-R"
...
Private Sub cmd_Gate_Click()
    ...
    If Trim(txt_to_area_name.Text) = "" Then
        Call Gp_MsgBoxDisplay(txt_to_area.Tag & "必须输入")
        Exit Sub
    End If
    
    If Mid(Trim(txt_mv_lst_no.Text), 2, 4) <> text_cur_area_code.Text & txt_to_area.Text Then
        Call Gp_MsgBoxDisplay("移拨码单号与起始区/目标区代码不一致,请确认")
        Exit Sub
    End If
...
'    text_cur_area_code = "00"
'    txt_to_area.Text = "WD"
    
    Call txt_to_area_KeyUp(0, 0)
...
Private Sub text_cur_area_code_DblClick()

    Call text_cur_area_code_KeyUp(vbKeyF4, 0)

End Sub

Private Sub text_cur_area_code_Change()
Dim S_CODE As String

    If text_cur_inv.Text = "00" Then
         S_CODE = "C0023"
    ElseIf text_cur_inv.Text = "WG" Then
        S_CODE = "C0026"
    End If
    If Len(Trim(text_cur_area_code.Text)) = text_cur_area_code.MaxLength Then
        text_cur_area_name.Text = Gf_ComnNameFind(M_CN1, S_CODE, text_cur_area_code.Text, 2)
    Else
      text_cur_area_name.Text = ""
    End If
End Sub

Private Sub text_cur_area_code_KeyUp(KeyCode As Integer, Shift As Integer)
...
Dim S_CODE As String
    ...
    If KeyCode = vbKeyF4 Then
         ...
         DD.rControl.Add Item:=text_cur_area_code
...
Private Sub ss2_DblClick(ByVal Col As Long, ByVal Row As Long)
    ...
    If Len(Trim(ss2.Text)) > 10 Then
        ...
        ss2.Col = SS2_FR_INV
        text_cur_area_code.Text = ss2.Text
        ss2.Col = SS2_TO_INV
        txt_to_area.Text = ss2.Text
...
Private Sub txt_to_area_DblClick()

    Call txt_to_area_KeyUp(vbKeyF4, 0)
    
End Sub

Private Sub txt_to_area_KeyUp(KeyCode As Integer, Shift As Integer)
  ...
  Dim S_CODE As String
   ...
   End If
    ...
    If KeyCode = vbKeyF4 Then
        ...
        DD.sWitch = "MS"
        DD.sKey = S_CODE
        DD.rControl.Add Item:=txt_to_area
    ...
    End If
        ...
        If Len(Trim(txt_to_area.Text)) = txt_to_area.MaxLength Then
            txt_to_area_name.Text = Gf_ComnNameFind(M_CN1, S_CODE, txt_to_area.Text, 2)
        Else
            txt_to_area_name.Text = ""
        End If
...
Private Sub ExcelPrn()
    ...
    Set Wb = xlApp.Workbooks.Open(App.Path & "\ACB5036C.xls")
    ...
    xlApp.Range("J3").Value = txt_to_area_name.Text
...
Private Sub SSCommand2_Click()
   ...
   Call Gp_ACB5036C_Excel(Me, Proc_Sc("Sc")("Spread"), lBlkcol1, lBlkcol2, lBlkrow1, lBlkrow2)
End Sub


Private Sub Gp_ACB5036C_Excel(Fm As Form, sPname As Variant, bLkcol1 As Long, bLkcol2 As Long, bLkrow1 As Long, bLkrow2 As Long)
...
Const xlthin = 2
    ...
    With sPname
        ...
        xlSheet.Range("C15").Value = "子板数"
        xlSheet.Range("C16").Value = "客户代码"
  
        xlSheet.Range("A17", "A18").Merge
        xlSheet.Range("B17", "D18").Merge
        ss1.Col = SS1_MV_DATE:           xlSheet.Range("B1").Value = ss1.Text
        ss1.Col = SS1_CUST_CD:           xlSheet.Range("B2").Value = ss1.Text
        ...
        ss1.Col = SS1_THK_AVG:           xlSheet.Range("B8").Value = ss1.Text
        ...
        ss1.Col = SS1_STDSPEC_ORG_KND:   xlSheet.Range("B15").Value = ss1.Text
        ss1.Col = SS1_STDSPEC_STLGRD:    xlSheet.Range("B16").Value = ss1.Text
        ss1.Col = SS1_PLATE_SIZE:        xlSheet.Range("B17").Value = ss1.Text
        ...
        ss1.Col = SS1_PLATE_CON:         xlSheet.Range("D15").Value = ss1.Text
        ss1.Col = SS1_CUST_CD1:          xlSheet.Range("D16").Value = ss1.Text
'
