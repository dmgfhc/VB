...
Begin VB.Form ACB6040C 
   BackColor       =   &H00E0E0E0&
   Caption         =   "板坯装车实绩录入_ACB6040C"
   ClientHeight    =   9225
   ClientLeft      =   120
   ClientTop       =   1515
   ClientWidth     =   15240
   ...
   Begin VB.ComboBox cbo_hcr 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "ACB6040C.frx":0000
      Left            =   6150
      List            =   "ACB6040C.frx":000D
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin Threed.SSPanel SSPanel1 
         ...
         Begin VB.ComboBox Cbo_trans_way 
            ...
            EndProperty
            Height          =   315
            ItemData        =   "ACB6040C.frx":006F
            Left            =   1335
            List            =   "ACB6040C.frx":0071
            TabIndex        =   35
            Top             =   510
            Width           =   1080
         End
      ...
      Begin FPSpread.vaSpread ss2 
         ...
         SpreadDesigner  =   "ACB6040C.frx":0073
      End
   End
   Begin InDate.ULabel ULabel2 
      Height          =   315
  ...
  Call Gp_Ms_Collection(dtp_ins_date_prod_date2, "p", " ", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
            ...
            Call Gp_Ms_Collection(stx_move_time, " ", " ", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
             ...
             Call Gp_Ms_Collection(sdb_slab_wgt, " ", " ", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
                  Call Gp_Ms_Collection(cbo_hcr, "p", " ", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
    ...
    Call Gp_Sp_Collection(ss2, 24, " ", " ", " ", "i", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1, False)
    Call Gp_Sp_Collection(ss2, 25, " ", " ", " ", "i", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1, False)
    Call Gp_Sp_Collection(ss2, 26, " ", " ", " ", "i", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1, False)
    
    sc1.Add Item:=ss2, Key:="Spread"
    sc1.Add Item:="ACB6040C.P_SREFER2", Key:="P-R"
    sc1.Add Item:="ACB6040C.P_MODIFY2", Key:="P-M"
...
Private Sub Form_Load()
    ...
    Cbo_trans_way.AddItem "T:汽运"
    Cbo_trans_way.AddItem "R:铁运"
    Cbo_trans_way.AddItem "G:过跨"
...
Dim sQuery As String
    ...
    sQuery = "SELECT COUNT(*) FROM CP_SLAB_MOVE_PLAN WHERE PRC_STS = 'A' AND FROM_INV = '" + Trim(text_cur_inv_code.Text) + "'"
...
'            ss2.Col = 7
'            If ss2.Text <> txt_plt.Text Then
'            ProYN = Gf_MessConfirm("目标库和计划目标库不一致，确定要保存吗？", "Q")
'                   If ProYN = False Then
'                      Exit Sub
'                   End If
'        End If
...
'
    ...
    For iRow = 1 To ss2.MaxRows
        ...
        If ss2.Text = "Update" Then
            ...
            ss2.Col = 26
            ss2.Text = Mid(Cbo_trans_way.Text, 1, 1)
            
            ss2.Col = 7
            If ss2.Text <> "" Then
                If ss2.Text <> txt_plt.Text Then
    '                ProYN = Gf_MessConfirm("目标库('" & Trim(txt_plt.Text) & "') 和计划目标库('" & ss2.Text & "')不一致，确定要保存吗？", "Q")
    '                    If ProYN = False Then
    '                       Exit Sub
    '                    End If
                    Call Gp_MsgBoxDisplay("目标库('" & Trim(txt_plt.Text) & "') 和计划目标库('" & ss2.Text & "')不一致")
                    Exit Sub
                End If
            End If
        End If
