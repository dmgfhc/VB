...
Begin VB.Form AHD0270C 
   Caption         =   "厂库别 配送库库存（入库）_AHD0270C"
   ClientHeight    =   3195
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   4680
   ...
   Begin FPSpread.vaSpread ss1 
      ...
      ProcessTab      =   -1  'True
      Protect         =   0   'False
      SpreadDesigner  =   "AHD0270C.frx":0000
   End
...
Call Gp_Ms_Collection(text_cur_inv_code, "p", "n", " ", " ", "r", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
   ...
   Call Gp_Sp_Collection(ss1, 17, " ", " ", " ", " ", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1)
    ...
    Sc1.Add Item:="AHD0270C.P_REFER", Key:="P-R"
...
Private Sub cmdReport_Click()
    ...
    If text_cur_inv_code = "" Then
       MsgBox "请先输入您要查询的仓库！", vbCritical, "系统提示信息"
       Exit Sub
...
Public Sub Form_Ref()
    ...
    sQuery = "{call AHD0270C.P_REFER ('" + dtp_yy_mm.RawData + "','" + text_cur_inv_code + "','" + txt_plt.Text + "','" + txt_prod_cd.Text + "')}"
...
Private Sub ExcelPrn()
    ...
    If Trim(TXT_CHECK_YARD) <> Trim(text_cur_inv_code) Then
       MsgBox "选择的仓库没有进行查询，请先查询数据再打印！", vbCritical, "系统提示信息"
       Exit Sub
    ...
    xlApp.Workbooks.Open (App.Path & "\AHD0270C.xls")
