...
Begin VB.Form CEG1050C 
   Caption         =   "炼钢厂均衡查询/调整_CEG1050C"
   ClientHeight    =   9210
   ClientLeft      =   105
   ClientTop       =   1845
   ClientWidth     =   15315
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin SSSplitter.SSSplitter SSSplitter2 
         ...
         Begin FPSpread.vaSpread ss2 
            ...
            SpreadDesigner  =   "CEG1050C.frx":00A4
         End
      End
      Begin FPSpread.vaSpread ss1 
         ...
         RetainSelBlock  =   0   'False
         RowHeaderDisplay=   0
         SpreadDesigner  =   "CEG1050C.frx":1154
      End
   End
   Begin Threed.SSCommand cmd_slab_add 
      Height          =   435
      Left            =   12570
      TabIndex        =   18
      TabStop         =   0   'False
...
'-- Sub_System Name
'-- Program Name      MONTHLY SLAB USE PLAN SMS CHANGE
'-- Program ID        CEG1050C
...
Dim Active_Row As Long              'Active Row
Dim Active_Col As Long              'Active Col
...
Private Sub Form_Define()
    ...
    'Call Spread_Collection("Column_Num", "p(primary)", "n(Necessary)", "m(maxlength)", "i(insert)", "a(append_down), "l(lock)")
     ...
     Call Gp_Sp_Collection(ss2, 2, "p", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2, False)
     Call Gp_Sp_Collection(ss2, 3, " ", " ", " ", " ", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)
     Call Gp_Sp_Collection(ss2, 4, " ", " ", " ", " ", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)
     Call Gp_Sp_Collection(ss2, 5, "p", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2, False)
     ...
     Call Gp_Sp_Collection(ss2, 9, "p", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2, False)
    Call Gp_Sp_Collection(ss2, 10, "p", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2, False)
    Call Gp_Sp_Collection(ss2, 11, "p", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2, False)
    
    For iCol = 12 To ss2.MaxCols
        Call Gp_Sp_Collection(ss2, iCol, " ", " ", " ", " ", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)
    Next iCol
    
    'Spread_Collection
    sc2.Add Item:=ss2, Key:="Spread"
    sc2.Add Item:="CEG1050C.P_REFER2", Key:="P-R"
    sc2.Add Item:="CEG1050C.P_ONEROW2", Key:="P-O"
    sc2.Add Item:="CEG1050C.P_MODIFY2", Key:="P-M"
...
Private Sub cmd_Confirm_Click()
    ...
    If udt_yymm.RawData = "" Or Len(udt_yymm.RawData) <> 6 Then
        Call Gp_MsgBoxDisplay(udt_yymm.Tag + "必须输入", "", "错误提示")
        Exit Sub
    End If
    ...
    sQuery = "{call CEG1050C.P_PLAN_CONFIRM ('C3','" & udt_yymm.RawData & "','" & sUserID & "',?)}"
...
Private Sub cmd_slab_add_Click()

    If ss1.MaxRows = Active_Row Then Exit Sub
    If ss1.MaxCols - 1 = Active_Col Or ss1.MaxCols - 2 = Active_Col Or ss1.MaxCols = Active_Col Then Exit Sub
    If Active_Col Mod 3 = 0 Then Exit Sub
    
    Complete = False
    
    Load Slab_Add
    
    If Active_Col Mod 3 = 1 Then
        Slab_Add.txt_plt.Text = "B1"
    Else
        Slab_Add.txt_plt.Text = "B3"
    End If
    
    Slab_Add.txt_stlgrd.Text = txt_stlgrd.Text
    Slab_Add.sdb_slab_thk.Value = sdb_slab_thk.Value
    Slab_Add.sdb_slab_wid.Value = sdb_slab_wid.Value
...
Private Sub Form_Load()
    ...
    sStatus = Gf_CodeFind(M_CN1, "SELECT PRC_STS FROM ZP_JOB WHERE PLT = 'C3' AND MAIN_PGMID = 'CEG1000P'")
    If sStatus = "C" Then
        sAuthority = "0000"
        cmd_slab_design_change.Enabled = False
        cmd_slab_add.Enabled = False
...
Public Sub Form_Ref()
    ...
    sQuery1 = sQuery1 + "  FROM  EP_MON_REQ_SLAB "
    sQuery1 = sQuery1 + " WHERE  SLAB_FL  IN ('N', 'A') "
    sQuery1 = sQuery1 + " ORDER  BY SLAB_THK ASC "
    
    '炉次编制量标准 B1:150 B3:30
    
    'Data Display
    sQuery2 = " {call CEG1050C.P_DATA ()} "
    
    'STLGRD, WID SUM Display
    sQuery3 = " {call CEG1050C.P_STLGRD_WID ()} "
    
    'THK SUM Display
    sQuery4 = " {call CEG1050C.P_THK ()} "
    
    'SUM Display
    sQuery5 = " {call CEG1050C.P_TOTAL ()} "
    ...
    If SMESG = "OK" Then
        ...
        If SMESG = "OK" Then
            ...
            If Sp_Data_Refer1(ss1, sQuery2) Then     'SLAB Data Display
                ...
                opt_plt_no1.Value = True
                Call Gp_Sp_ReadOnlySet(sc1.Item("Spread"))
                sdb_slab_cnt1.Value = Gf_FloatFind(M_CN1, "SELECT SUM(ROUND(ORD_REM_SLAB_CNT)) FROM EP_MON_REQ_SLAB WHERE SMS_PLT = 'B1' AND SLAB_FL IN ('N','A') ")
                sdb_slab_cnt2.Value = Gf_FloatFind(M_CN1, "SELECT SUM(ROUND(ORD_REM_SLAB_CNT)) FROM EP_MON_REQ_SLAB WHERE SMS_PLT = 'B3' AND SLAB_FL IN ('N','A') ")
                sdb_slab_wgt1.Value = Gf_FloatFind(M_CN1, "SELECT SUM(NVL(ORD_REM_SLAB_WGT,0)) FROM EP_MON_REQ_SLAB WHERE SMS_PLT = 'B1' AND SLAB_FL IN ('N','A') ")
                sdb_slab_wgt2.Value = Gf_FloatFind(M_CN1, "SELECT SUM(NVL(ORD_REM_SLAB_WGT,0)) FROM EP_MON_REQ_SLAB WHERE SMS_PLT = 'B3' AND SLAB_FL IN ('N','A') ")
                sdb_slab_cnt3.Value = sdb_slab_cnt1.Value + sdb_slab_cnt2.Value
                sdb_slab_wgt3.Value = sdb_slab_wgt1.Value + sdb_slab_wgt2.Value
            End If
...
Private Sub ss2_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    If ss2.Text = "N" Or ss2.Text = "A" Then
    Else
        Exit Sub
    End If
