...
Begin VB.Form AKP1016C 
   Caption         =   "炼钢日成本核算(按炉号)_AKP1016C"
   ClientHeight    =   9225
   ClientLeft      =   315
   ClientTop       =   2655
   ClientWidth     =   15225
   ...
   Begin VB.ComboBox cbo_prc_line 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "AKP1016C.frx":0000
      Left            =   6780
      List            =   "AKP1016C.frx":000D
      TabIndex        =   1
      Tag             =   "连铸机号"
      Top             =   105
      Width           =   750
   ...
   Begin FPSpread.vaSpread ss1 
      ...
      SpreadDesigner  =   "AKP1016C.frx":001A
   End
...
'---------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    sc1.Add Item:="AKP1016C.P_REFER", Key:="P-R"
...
Private Sub Cmd_Edit_Click()
    ...
    Response = MsgBox("生成" + Mid(txt_from_DATE.RawData, 1, 4) + "年" + Mid(txt_from_DATE.RawData, 5, 2) + "月" + Mid(txt_from_DATE.RawData, 7, 2) + "日  " + "新报表吗?", vbYesNo, "系统提示信息")
    ...
    adoCmd.CommandText = "{call AKP1016P ('" + txt_from_DATE.RawData + "')}"
...
Process_Exec_ERROR:
    ...
    Set adoCmd = Nothing
    Call Gp_MsgBoxDisplay(Err.Description & "{call AKP1016P ('" + txt_from_DATE.RawData + "')}")
...
Public Sub Form_Ref()
    ...
    If Gf_Sp_Refer(M_CN1, Proc_Sc("Sc"), Mc1, Mc1("nControl"), Mc1("mControl")) Then
        ...
        slab_wgt = Pf_Sp_ColSum(ss1, 6, 1, ss1.MaxRows)
        ...
        ss1.Col = 6: ss1.Text = Str(slab_wgt)
