...
'-- 1.01  2003.09.08  Yang Zhibin   物料库存现状查询
'-- 1.02  2011.05.24  LiQian        物料库存现状查询，增加按轧制日期查询条件
'-------------------------------------------------------------------------------
'-- DECLARATION     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
'-------------------------------------------------------------------------------
...
Const SPD_DATE_CD = 25  '生产日期/剪切日期    / 轧制日期   23->25
Const SPD_SPEC = 7  '标准
...
Public Sub Form_Ref()                   '###################################################################################################
    ...
    If Refer_Fl = "Y" Then
        If Len(txt_mat_no.Text) < 8 And Len(txt_lot_no.Text) < 12 And txt_ord_no.Text = "" And cbo_ord_item.Text = "" And (DTP_PROD_FR.RawData = "" Or DTP_PROD_TO.RawData = "") Then
            Call Gp_MsgBoxDisplay("生产日期或订单号或物料号或轧批号必须输入", "I", "错误提示")
            Exit Sub
        End If
    End If
...
'On Error GoTo Process_Exec_ERROR
    ...
    If ss1.MaxRows <= 0 Then Exit Sub
     ...
     If Trim(txt_woo_rsn.Text) = "" Then
        Call Gp_MsgBoxDisplay(txt_woo_rsn.Tag + "必须输入")
        Exit Sub
    End If
    
    If Len(Trim(txt_woo_rsn.Text)) <> txt_woo_rsn.MaxLength Then
         Call Gp_MsgBoxDisplay(txt_woo_rsn.Tag + " 输入错误")
        Exit Sub
    End If
    
    If Trim(txt_rep_remark.Text) = "" Then
        Call Gp_MsgBoxDisplay(txt_rep_remark.Tag + "必须输入")
        Exit Sub
    End If
    
'    If Len(Trim(txt_rep_remark.Text)) > txt_rep_remark.MaxLength Then
'         Call Gp_MsgBoxDisplay(txt_rep_remark.Tag + " 50 个字符内")
'        Exit Sub
'    End If
...
'            .Text = ""
    ...
    If inum = 0 Then
        Call Gp_MsgBoxDisplay("没有选定的物料!!", "I")
        Set adoCmd = Nothing
        Screen.MousePointer = vbDefault
        Exit Sub
    End If
