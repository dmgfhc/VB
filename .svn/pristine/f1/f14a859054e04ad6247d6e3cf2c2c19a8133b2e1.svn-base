...
Begin VB.Form CEF1511C 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "板坯替代计划/分配余量_CEF1511C"
   ClientHeight    =   4125
   ClientLeft      =   435
   ClientTop       =   3030
   ClientWidth     =   13965
   ...
   Begin FPSpread.vaSpread ss1 
      ...
      ProcessTab      =   -1  'True
      Protect         =   0   'False
      SpreadDesigner  =   "CEF1511C.frx":0000
   End
...
Private Sub Cmd_Cancel_Click()
    ...
    sdb_slab_rem_len.Value = sdb_slab_org_rem_len.Value
    sdb_slab_rem_len1.Value = sdb_slab_org_rem_len.Value
    ...
    For iRow = 1 To ss1.MaxRows
        ...
        If Clear_Fl Then
            ss1.Col = 1
            ss1.Value = 0
            Call Gp_Sp_BlockLock(ss1, 11, 11, iRow, iRow, True)
        Else
            ss1.Col = 1
            ...
            If ss1.Value = 0 Then
                Call Gp_Sp_BlockLock(ss1, 11, 11, iRow, iRow, True)
            Else
                Call Gp_Sp_BlockLock(ss1, 11, 11, iRow, iRow, False)
            End If
        End If
...
Private Sub cmd_cnf_Click()
    ...
    For iRow = 1 To ss1.MaxRows
        ...
        CEF1510C.Chg_Slab_Len.Remove (ss1.MaxRows - iRow + 1)
        CEF1510C.Chg_Slab_Wgt.Remove (ss1.MaxRows - iRow + 1)
        CEF1510C.Add_Slab_Len.Remove (ss1.MaxRows - iRow + 1)
        CEF1510C.Add_Slab_Wgt.Remove (ss1.MaxRows - iRow + 1)
    ...
    For iRow = 1 To ss1.MaxRows
        ...
        If ss1.Value = 1 Then
            ...
            If IIf(ss1.Text = "", 0, ss1.Value) <> 0 Then
                ...
                CEF1510C.sdb_slab_rem_len.Value = sdb_slab_rem_len1.Value
                CEF1510C.sdb_slab_rem_wgt.Value = sdb_slab_rem_wgt1.Value
        ...
        CEF1510C.Chg_Slab_Len.Add Item:=Chg_Slab_Len(iRow)
        CEF1510C.Add_Slab_Len.Add Item:=Add_Slab_Len(iRow)
        CEF1510C.Chg_Slab_Wgt.Add Item:=Chg_Slab_Wgt(iRow)
        CEF1510C.Add_Slab_Wgt.Add Item:=Add_Slab_Wgt(iRow)
        
        CEF1510C.lbl_slab(iRow).Width = (CEF1510C.Shape1.Width / CEF1510C.sdb_slab_wgt.Value) * (Chg_Slab_Wgt.Item(iRow) + IIf(iRow > 0, CEF1510C.dSlab_Cut_Spare_Wgt, 0))
        
        If iRow = 1 Then
            CEF1510C.lbl_slab(iRow).Left = CEF1510C.Shape1.Left
        Else
            CEF1510C.lbl_slab(iRow).Left = CEF1510C.lbl_slab(iRow - 1).Left + CEF1510C.lbl_slab(iRow - 1).Width
        End If
        
        'ToolTipText Setting
        CEF1510C.lbl_slab(iRow).ToolTipText = CEF1510C.Ord_No.Item(iRow) & "-" & CEF1510C.Ord_Item.Item(iRow) & "(" & CEF1510C.Chg_Slab_Len.Item(iRow) & "," & CEF1510C.Chg_Slab_Wgt.Item(iRow) & ")"
...
Private Sub cmd_process_Click()
    ...
    dSlab_Len = Gf_FloatFind(M_CN1, "SELECT TRUNC(" & sdb_slab_rem_len.Value & "/" & iChk & ") FROM DUAL")
    ...
    For iRow = 1 To ss1.MaxRows
        ...
        If ss1.Value = 1 Then
            ...
            ss1.Col = 15
            sStlgrd = ss1.Text
            dSlab_Wgt = Gf_FloatFind(M_CN1, "SELECT GF_JP_WGT('WGT','" & sStlgrd & "'," & sdb_slab_thk.Value & "," & sdb_slab_wid.Value & "," & dSlab_Len & ",0) FROM DUAL ")
            
            ss1.Col = 12
            ss1.Value = dSlab_Wgt
            sdb_slab_rem_wgt1.Value = sdb_slab_rem_wgt1.Value - dSlab_Wgt
            
            'AsRoll Thk
            sQuery = "SELECT MAX(ASROLL_THK) FROM NISCO.CP_ORD_SL_D "
            sQuery = sQuery & " WHERE  ORD_NO    =  '" & sOrd_No & "'"
            sQuery = sQuery & "   AND  ORD_ITEM  =  '" & sOrd_item & "'"
            
            dAsroll_Thk = Gf_FloatFind(M_CN1, sQuery)
            
            'AsRoll Wid
            sQuery = "SELECT MAX(ASROLL_WID) FROM NISCO.CP_ORD_SL_D "
            sQuery = sQuery & " WHERE  ORD_NO    =  '" & sOrd_No & "'"
            sQuery = sQuery & "   AND  ORD_ITEM  =  '" & sOrd_item & "'"
            
            dAsroll_Wid = Gf_FloatFind(M_CN1, sQuery)
    ...
    If sdb_slab_rem_len1.Value <> 0 And iLastRow <> 0 Then
    
        ss1.Row = iLastRow
        ss1.Col = 11
        ss1.Value = ss1.Value + sdb_slab_rem_len1.Value
        dSlab_Len = ss1.Value
        ss1.Col = 15
        dSlab_Wgt = Gf_FloatFind(M_CN1, "SELECT GF_JP_WGT('WGT','" & ss1.Text & "'," & sdb_slab_thk.Value & "," & sdb_slab_wid.Value & "," & dSlab_Len & ",0) FROM DUAL ")
        ss1.Col = 12
        ss1.Value = dSlab_Wgt
...
Private Sub ss1_EditMode(ByVal Col As Long, ByVal Row As Long, ByVal Mode As Integer, ByVal ChangeMade As Boolean)
    ...
    For iRow = 1 To ss1.MaxRows
        ...
        If ss1.Value = 1 Then
            ...
            Call Gp_Sp_BlockLock(ss1, 11, 11, iRow, iRow, False)
            Call Gp_Sp_CellColor(ss1, 11, iRow, , &HC0FFFF)
        Else
            ss1.Col = 11
            ss1.Text = ""
            ss1.Col = 12
    ...
    With ss1
        ...
        Select Case Col
            ...
            Case 11     'SLAB_LEN
                ...
                If Abs(dSlab_Len) > Round(Abs(sdb_slab_rem_len.Value), 3) Then
