...
Begin VB.Form CEC2050C 
   Caption         =   "炼钢厂均衡查询/调整_CEC2050C"
   ClientHeight    =   9225
   ClientLeft      =   285
   ClientTop       =   1965
   ClientWidth     =   15300
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin SSSplitter.SSSplitter SSSplitter2 
         ...
         Begin FPSpread.vaSpread ss2 
            ...
            SpreadDesigner  =   "CEC2050C.frx":00A4
         End
      End
      Begin FPSpread.vaSpread ss1 
         ...
         RetainSelBlock  =   0   'False
         RowHeaderDisplay=   0
         SpreadDesigner  =   "CEC2050C.frx":1288
      End
   End
...
Private Sub Form_Define()
    ...
    sc2.Add Item:="CEC2050C.P_REFER2", Key:="P-R"
    sc2.Add Item:="CEC2050C.P_ONEROW2", Key:="P-O"
    sc2.Add Item:="CEC2050C.P_MODIFY2", Key:="P-M"
    ...
    Call Gp_Sp_ColHidden(ss2, 4, True)
...
Public Sub Form_Ref()
    ...
    sQuery1 = sQuery1 + "  FROM  EP_C3_REQ_SLAB_EDT "
    sQuery1 = sQuery1 + " ORDER  BY SLAB_THK ASC "
    
    '炉次编制量标准 B1:150 B3:30
    
    'Data Display
    sQuery2 = " {call CEC2050C.P_DATA ()} "
    
    'STLGRD, WID SUM Display
    sQuery3 = " {call CEC2050C.P_STLGRD_WID ()} "
    
    'THK SUM Display
    sQuery4 = " {call CEC2050C.P_THK ()} "
    
    'SUM Display
    sQuery5 = " {call CEC2050C.P_TOTAL ()} "
    ...
    If SMESG = "OK" Then
        ...
        If SMESG = "OK" Then
            ...
            If Sp_Data_Refer1(ss1, sQuery2) Then     'SLAB Data Display
                ...
                opt_plt_no1.Value = True
                Call Gp_Sp_ReadOnlySet(sc1.Item("Spread"))
                sdb_slab_cnt1.Value = Gf_FloatFind(M_CN1, "SELECT COUNT(SLAB_EDT_SEQ) FROM EP_C3_REQ_SLAB_EDT WHERE SMS_PLT = 'B1'  ")
                sdb_slab_cnt2.Value = Gf_FloatFind(M_CN1, "SELECT COUNT(SLAB_EDT_SEQ) FROM EP_C3_REQ_SLAB_EDT WHERE SMS_PLT = 'B3'  ")
                sdb_slab_wgt1.Value = Gf_FloatFind(M_CN1, "SELECT SUM(NVL(SLAB_WGT,0)) FROM EP_C3_REQ_SLAB_EDT WHERE SMS_PLT = 'B1'  ")
                sdb_slab_wgt2.Value = Gf_FloatFind(M_CN1, "SELECT SUM(NVL(SLAB_WGT,0)) FROM EP_C3_REQ_SLAB_EDT WHERE SMS_PLT = 'B3'  ")
            End If
