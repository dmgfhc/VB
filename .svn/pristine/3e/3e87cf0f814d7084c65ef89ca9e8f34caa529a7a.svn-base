...
Function GetChem_Rslt_SQL(ByVal sCert_No As String) As String
    ...
    If adoCmd("arg_SQL") = "NN" Then
        ret_Result_ErrMsg = "获取失败，请检查质保书内容"
...
Private Function funGetQuery_C(sCertNo As String, iSave_State As Integer, sSave_Path As String) As String
    ...
    sQuery = sQuery + ",'JB4730-94 Ⅰ级','JB4730 J21','JB4730-94 Ⅱ级','GB/T 2970 K11','GB/T 2970 Ⅰ级','GB/T 2970 K21','GB/T 2970 Ⅱ级','NO UST',' '"
...
'--------------------------------------------------------------------------------------------------------------------------------------
    ...
    ReDim pAry11(1 To 6, 1 To 6)                                    '产品信息(产品号\牌号\尺寸\支数\重量)
    ReDim pAry12(1 To 1, 1 To 18)                                   '成分名称
    ReDim pAry13(1 To 1, 1 To 18)                                   '成分倍数
    ReDim pAry14(1 To 5, 1 To 18)                                   '成分实际
    ReDim pAry15(1 To 1, 1 To 2)                                    '备用成分名称
    ReDim pAry16(1 To 1, 1 To 2)                                    '备用成分倍数
    ReDim pAry17(1 To 5, 1 To 2)                                    '备用成分实际
    
    ReDim pAry2(1 To 5, 1 To 23)                                    '材质实际(第一位产品号)
'--------------------------------------------------------------------------------------------------------------------------------------
...
'2008,3,14 sunbin start
             If Mid(Trim(arrRecords1(9, 0)), 1, 13) = "PCLZCPO030035" Then
                    xlSheet.Range("H32").Value = "工程名称：兰郑长成品油管道工程 承包商名称：兰郑长EPC项目部"
...
'
            ...
            ReDim pAry11(1 To 5, 1 To 6)                                    '产品信息(产品号\牌号\尺寸\支数\重量)
            ReDim pAry12(1 To 1, 1 To 18)                                   '成分名称
            ReDim pAry13(1 To 1, 1 To 18)                                   '成分倍数
            ReDim pAry14(1 To 5, 1 To 18)                                   '成分实际
            ReDim pAry15(1 To 1, 1 To 2)                                    '备用成分名称
...
'        sLicence_NO = "特种设备制造许可证：" & arrRecords1(3, 0)
...
'---------------- print ---------------------------------------------------------------------------------
    ...
    If InStr(1, Trim(s_MyPono), "西门子") > 0 Or InStr(1, Trim(s_MyPono), "YJ0801008") > 0 Then
        xlSheet.Range("J31").Value = "质检负责人" + Chr$(13) + Chr$(10) + "Chief  Inspector"
    ElseIf InStr(1, Trim(s_MyPono), "西门子") > 0 Or InStr(1, Trim(s_MyPono), "YJ0802002") > 0 Then
        xlSheet.Range("J31").Value = "质检负责人" + Chr$(13) + Chr$(10) + "Chief  Inspector"
    ElseIf InStr(1, Trim(s_MyPono), "西门子") > 0 Or InStr(1, Trim(s_MyPono), "YJ0803008") > 0 Then
        xlSheet.Range("J31").Value = "质检负责人" + Chr$(13) + Chr$(10) + "Chief  Inspector"
    Else
...
'---------------------  协议 -----------------------------------------
    If Trim(arrRecords1(20, 0) & "") = "3" Then
        xlSheet.Range("B2").Value = "协议书号："
        xlSheet.Range("B3").Value = "Concert No."
        xlSheet.Range("C4").Value = arrRecords1(1, 0) & "(协议品)"        'PROD_NAME
        xlSheet.Range("I5").Value = "协 议 证 明 书"
        xlSheet.Range("H7").Value = "Agreement Certificate"
        xlSheet.Range("C6").Value = "用户协议"
        xlSheet.Range("D30").Value = "协议产品"
        xlSheet.Range("D31").Value = "质量说明章"
    End If
    
    If UCase(sPROD_CD) = "HC" Then
        xlSheet.Range("B14").Value = "产品号" + Chr$(10) + "Coil №"
        xlSheet.Range("B22").Value = "产品号" + Chr$(10) + "Coil №"
    Else
        xlSheet.Range("B14").Value = "产品号" + Chr$(10) + "Plate №"
        xlSheet.Range("B22").Value = "产品号" + Chr$(10) + "Plate №"
    End If
...
Private Function funGetQuery_S(ByVal sCertNo As String, ByVal iSave_State As Integer, ByVal sSave_Path As String)
    ...
    Set AdoRs = New adodb.Recordset
'    增加客户名称 耿学玉 2011 0615
'    增加订单备注（根据订单备注是否有“执行新规范”决定是否打印旧船规注释） 刘翔   2011 1009
'    增加硬度试验要求,冷弯 耿学玉 20120221
...
Private Function MillSheetPrint_S(ByVal iSave_State As Integer, ByVal sSave_Path As String, ByVal arrRecords1 As Variant, ByVal arrRecords2 As Variant) As String
    ...
    Dim dAry()      As Double  '   为重量服务 防止小于1t 出现 .XXX
...
'    End If
    ...
    Do
        ...
        pAry(LneCnt, 2) = Mid(arrRecords2(1, PrtCnt), 1, 8) & ""       ' 炉号
        pAry(LneCnt, 3) = arrRecords2(3, PrtCnt) & ""                  ' PRDT_QNTY
'        耿学玉  规格
        pAry(LneCnt, 4) = arrRecords2(2, PrtCnt) & ""                      ' 规格
'        防止重量小于1t 出现.XXX 并且把模板改为数字
    ...
    '        灵活显示追加的试验要求  耿学玉 20120221
            
                
            If sADD_MATR = "A" Then
                        pAry(LneCnt, 37) = IIf(arrRecords2(48, PrtCnt) = 0, "-", arrRecords2(48, PrtCnt) & "")    '船板质保书追加打印硬度
    '        ElseIf sADD_MATR = "B" Then
    '                    pAry(LneCnt, 35) = IIf(arrRecords2(49, PrtCnt) = 0, "-", arrRecords2(49, PrtCnt) & "")    '  冷弯打印
            Else
                        pAry(LneCnt, 37) = "-"                          ' 空白-
            End If
    ...
    '        灵活显示追加的试验要求  耿学玉 20120221
            ...
            If sADD_MATR = "A" Then
                        pAry(LneCnt, 35) = IIf(arrRecords2(48, PrtCnt) = 0, "-", arrRecords2(48, PrtCnt) & "")    '船板质保书追加打印硬度
    '        ElseIf sADD_MATR = "B" Then
    '                    pAry(LneCnt, 35) = IIf(arrRecords2(49, PrtCnt) = 0, "-", arrRecords2(49, PrtCnt) & "")    '  冷弯打印
            Else
                        pAry(LneCnt, 35) = "-"                          ' 空白-
...
'20100611 sun bin  end
        ...
        dSumWGT = IIf(dSumWGT < 1, "0" & dSumWGT, dSumWGT)              '防止重量小于1t 出现.XXX 并且把模板改为数字

        lSumQNTY_T = lSumQNTY_T + arrRecords2(3, PrtCnt)                'TOTAL SUM QUANTITY
        dSumWGT_T = dSumWGT_T + arrRecords2(4, PrtCnt)                  'TOTAL SUM WEIGHT
        dSumWGT_T = IIf(dSumWGT_T < 1, "0" & dSumWGT_T, dSumWGT_T)      '防止重量小于1t 出现.XXX 并且把模板改为数字
        ...
        If LneCnt = 5 Then
            ...
            If PrtCnt = RowCNT Then
                xlSheet.Range("B42").Value = "合计(Total)"
...
'第二个试样号
            If arrRecords1(8, 0) = "NV" And Trim(pAry1(2, 1)) = "" Then  'NV试样号、产品号为空则打印“-” 20110812 liuxiang
...
'                xlSheet.Range("C28").Value = Mid(pAry1(2, 2), 331, Len(pAry1(2, 2)) - 331)
            End If
'第三个试样号
            If arrRecords1(8, 0) = "NV" And Trim(pAry1(3, 1)) = "" Then  'NV试样号、产品号为空则打印“-” 20110812 liuxiang
...
Private Sub MillSheetPrint_S_Head(ByVal arrRecords1 As Variant)
    ...
    Dim sPROD_CD As String
    Dim sSTDSPEC As String
    Dim sNEWSPEC As String  '用于根据订单备注判断是否执行新船规   刘翔 2011.10.9
    Dim sADD_MATR As String  '追加性能质保打印 硬度、冷弯 硬度优先，再者冷弯
    
    '如果是说明书，一下内容不打印
    If Report_KND = "T" Then
        arrRecords1(0, 0) = ""      '质保书号
        arrRecords1(1, 0) = ""      '工厂许可号
        arrRecords1(12, 0) = ""     '控制号
    End If
    
    sDate = GetPrintDate()
    sCert_No = arrRecords1(0, 0)
    sPROD_CD = Mid(sCert_No, 1, 2)
    sNEWSPEC = IIf(InStr(arrRecords1(27, 0), "执行新规范") = 0, "0", "1") '"0"：旧船规  "1"：新船规
...
'    xlSheet.Range("C9").Value = arrRecords1(3, 0) & ""        'COND_SUPPLY
    ...
    '如果是NV，牌号前加“NV ” 2012.5.23 刘翔
    If arrRecords1(8, 0) = "NV" Then
        xlSheet.Range("E11").Value = "NV " + arrRecords1(4, 0) + ")" + " 冲击试样尺寸（mm):" + arrRecords1(5, 0)          'PROD_NAME1
    ...
    ElseIf arrRecords1(8, 0) = "LR" Then
        xlSheet.Range("E11").Value = arrRecords1(4, 0) + ")" + " 冲击试样尺寸（mm):" + arrRecords1(5, 0)            'PROD_NAME1
...
'    ElseIf arrRecords1(8, 0) = "RS" Then ' RS 加 PC Guhf
'        xlSheet.Range("E11").Value = "PC" + arrRecords1(4, 0) + ")" + " 冲击试样尺寸（mm):" + arrRecords1(5, 0)          'PROD_NAME1
'        xlSheet.Range("G12").Value = arrRecords1(4, 0) + " Dimensions of test specimens:" + arrRecords1(5, 0) + ")"  'PROD_NAME2
'        xlSheet.Range("C6").Value = sSTDSPEC 'STDSPEC_NAME
'        xlSheet.Range("C9").Value = arrRecords1(3, 0) & ""        'COND_SUPPLY
    Else
        ...
        xlSheet.Range("E11").Value = Replace(arrRecords1(4, 0), "N", "") + ")" + " 冲击试样尺寸（mm):" + arrRecords1(5, 0)          'PROD_NAME1
...
'放到质保书基本信息里 耿学玉  2011 06 23
'    xlSheet.Range("H13").Value = arrRecords1(24, 0) & ""        'PROD_SIZE
'  --WCH 2013-01-11 CCS质保书不同格式
   If UCase(Mid(arrRecords1(8, 0), 1, 3)) = "ABS" Then
     xlSheet.Range("AG6").Value = Left(sDate, 4) + "年" + Mid(sDate, 6, 2) + "月" + Mid(sDate, 9, 2) + "日"          'CERT_RPT_DATE1
     xlSheet.Range("AG7").Value = Mid(sDate, 6, 2) + "/" + Mid(sDate, 9, 2) + "/" + Left(sDate, 4)          'CERT_RPT_DATE2
   Else
     xlSheet.Range("AE6").Value = Left(sDate, 4) + "年" + Mid(sDate, 6, 2) + "月" + Mid(sDate, 9, 2) + "日"          'CERT_RPT_DATE1
     If arrRecords1(8, 0) = "NV" Then
...
'    xlSheet.Range("I13").Value = "-" & ""                           'UST_STD 如果NV无探伤打印“-” 20110812 liuxiang
'Else
...
End If
    ...
    'xlSheet.Range("V13").Value = arrRecords1(26, 0) & ""            '客户名称
    xlSheet.Range("V13").Value = arrRecords1(26, 0) + Chr(10) + "订单号：" + arrRecords1(33, 0) + "-" + arrRecords1(34, 0) & ""      '客户名称
'20100611 sun bin end
    ...
    If arrRecords1(8, 0) = "CCS" Or arrRecords1(8, 0) = "RINA" Then
        ...
        xlSheet.Range("B39").Value = arrRecords1(15, 0) & ""                                'TEXT3 CCS 不打CEQ公式
'        xlSheet.Range("B40").Value = arrRecords1(16, 0) & arrRecords1(4, 0) & " steel."     'TEXT4
        xlSheet.Range("B40").Value = arrRecords1(16, 0) & ""                                 'TEXT4
        Else
        xlSheet.Range("B39").Value = arrRecords1(30, 0) & ""                                'TEXT3 改成 根据订单成份标准的公式 灵活抓取公式 耿学玉 20120224
        xlSheet.Range("B40").Value = arrRecords1(16, 0) & arrRecords1(4, 0) & " steel."     'TEXT4
        End If
    Else
...
'    xlSheet.Range("B37").Value = arrRecords1(16, 0) & ""      'TEXT4
    ...
    Else
'        xlSheet.Range("B41").Value = IIf(sNEWSPEC = "1", "", arrRecords1(17, 0) & "")  'TEXT5  根据sNEWSPEC判断是否打印
        xlSheet.Range("G37").Value = "                                       " + arrRecords1(18, 0) & ""    'TEXT6
        ...
        End If
'        xlSheet.Range("G41").Value = IIf(sNEWSPEC = "1", "", arrRecords1(22, 0) & "") 'TEXT10 根据sNEWSPEC判断是否打印
'        xlSheet.Range("W37").Value = IIf(arrRecords1(8, 0) = "NV", "DNV", arrRecords1(8, 0)) & ""       'STD_ORGAN
        If UCase(Mid(arrRecords1(8, 0), 1, 3)) = "ABS" Then
...
'    xlSheet.Range("K41").Value = IIf(sNEWSPEC = "1", "", arrRecords1(22, 0) & "") 'TEXT10 根据sNEWSPEC判断是否打印
...
'    End If
    ...
    If sADD_MATR = "A" Then
      If UCase(Mid(arrRecords1(8, 0), 1, 3)) = "ABS" Then
                xlSheet.Range("AJ14").Value = "硬度" & Chr(10) & "HRC"   '船板质保书追加打印硬度
      Else
               xlSheet.Range("AL14").Value = "硬度" & Chr(10) & "HRC"   '船板质保书追加打印硬度
      End If
'    ElseIf sADD_MATR = "B" Then
'                xlSheet.Range("AJ14").Value = "冷弯" & Chr(10) & "B.D."    '  冷弯打印
    Else
        If UCase(Mid(arrRecords1(8, 0), 1, 3)) = "ABS" Then
                xlSheet.Range("AJ14").Value = "-"                          ' 空白-
        Else
                xlSheet.Range("AL14").Value = "-"                          ' 空白-
        End If
    End If
...
'    End If
    ...
    '有侧喷加喷就显示船厂信息 2015.1.28 刘翔
    '改为取加喷信息（徐兴田） 2015.8.31 刘翔
    If Trim(arrRecords1(31, 0)) <> "" Then
       xlSheet.Range("T12").Value = "加喷信息:" + arrRecords1(31, 0) & ""
'       If UCase(Mid(arrRecords1(8, 0), 1, 3)) = "ABS" Then
'          xlSheet.Range("Y12").Value = arrRecords1(31, 0) & ""
'       Else
'          xlSheet.Range("W12").Value = arrRecords1(31, 0) & ""
...
Private Function funGetQuery_P(sCertNo As String, iSave_State As Integer, sSave_Path As String) As String
    ...
    sQuery = sQuery + ", DECODE(GRAIN_SIZE_RST,NULL,0,GRAIN_SIZE_RST),DECODE(NON_METAL_DSC_RST,'Y','OK','N','NO'),BELT_STR_DSC_RST" ','Y','合格','N','不合格')"
    sQuery = sQuery + " FROM QP_CERT_DETAIL WHERE CERT_NO  = '" & sCertNo & "'"
...
'            xlSheet.Range("J10").Value = arrRecords3(25, PrtCnt)
            ...
            End If
            If PrtCnt = RowCNT Then
                xlSheet.Range("Q28").Value = "合计(Total)"
                xlSheet.Range("Q29").Value = "件数  Piece"              'SUM_CNT UNIT
                xlSheet.Range("Q30").Value = lSumQNTY_T                 'SUM_CNT
                xlSheet.Range("R29").Value = "重量(mt)   Weight"        'SUM_WGT UNIT
...
'--- PRINT ----------------------------------------------------------------------------------------------
    ...
    If UCase(sPROD_CD) = "HC" Then
        xlSheet.Range("B12").Value = "产品号" + Chr$(10) + "Coil №"
        xlSheet.Range("B20").Value = "产品号" + Chr$(10) + "Coil №"
    Else
        xlSheet.Range("B12").Value = "产品号" + Chr$(10) + "Plate №"
        xlSheet.Range("B20").Value = "产品号" + Chr$(10) + "Plate №"
    End If
    
    If Mid(sPONO, 1, 10) = "07JTE/P003" Then
       xlSheet.Range("C6").Value = "技术协议"
    End If
    
' SUN BIN START 20080522
    
    If Trim(arrRecords1(1, 0)) = "API SPEC 5L-2000" Then
        xlSheet.Range("C6").Value = "API SPEC 5L (43 版)" & ""        'PROD_SPEC_NO
    Else
...
' SUN BIN END 20080522
    ...
    xlSheet.Range("B10").Value = "交货状态： " & sCONDITION_CN & "" 'COND_SUPPLY
...
Public Function funGetQuery_SPCL(ByVal sPacket_No As String, ByVal sSave_Path As String)
    ...
    sCert_No = Format(Date, "YYMMDD") & "T" & sPacket_No 'Format(sPacket_No, "00000")
      ...
      STDSPEC = arrRecords1(1, 0)     '交货状态
      THK = Mid(arrRecords1(3, i), 1, InStr(arrRecords1(3, i), "*")) '厚度
      
      
      If InStr(STDSPEC, "S460") > 0 Or InStr(STDSPEC, "TB460") > 0 Then
        COND_SUPPLY = "交货状态: 正火+高温回火"
      End If
      
      If InStr(STDSPEC, "S500") > 0 Or InStr(STDSPEC, "TB500") > 0 Then
      xlSheet.Range("J23").Value = "ReL，MPa"
        If Val(THK) > 16 Then
          COND_SUPPLY = "交货状态: 淬火+高温回火"
        Else
          COND_SUPPLY = "交货状态: TMCP+高温回火"
        End If
      End If
...
'---- EN10020-6-S620QL1 -----------------------------------------------------------------------------------
    If arrRecords1(1, 0) = "EN 10025-6-S620QL1" Or arrRecords1(1, 0) = "JX/NG-0001-TB620" Or arrRecords1(1, 0) = "JX/NG-0002-TB620" Then
      ...
      xlSheet.Range("O35").Value = arrRecords1(31, 0) '冲击尺寸
      xlSheet.Range("C33").Value = RowCNT / 2 & "块"  '总计块数
      xlSheet.Range("AB8").Value = Date
      xlSheet.Range("AB6").Value = sCert_No
      
      Bend_str = "弯曲角度=" + Str(arrRecords1(51, 0)) + "°" + "；弯心直径=" + Str(arrRecords1(50, 0)) + "a；试样宽度≥7a"
      ...
      j = 0                        '质保书成分行
      II = i                       '质保书性能行
      For i = 0 To RowCNT - 1      '记录数

      '成分只取单数行记录
    
        If i Mod 2 <> 0 Then
          xlSheet.Cells(j + 14, 1).Value = arrRecords1(2, i)    '产品号
          xlSheet.Cells(j + 14, 3).Value = arrRecords1(3, i)    '规格
          xlSheet.Cells(j + 14, 7).Value = arrRecords1(4, i)    '尺寸
          xlSheet.Cells(j + 14, 8).Value = arrRecords1(32, i)   '重量
        ...
        xlSheet.Cells(II + 21, 7).Value = arrRecords1(5, i)   '试样号
        xlSheet.Cells(II + 21, 8).Value = arrRecords1(20, i)   '取样位置
        ...
        xlSheet.Cells(II + 21, 17).Value = IIf(Val(arrRecords1(33, i) & "") = 0, "-", arrRecords1(33, i) & "")  ' Z向面缩1
        xlSheet.Cells(II + 21, 18).Value = IIf(Val(arrRecords1(34, i) & "") = 0, "-", arrRecords1(34, i) & "")  ' Z向面缩2
        xlSheet.Cells(II + 21, 19).Value = IIf(Val(arrRecords1(35, i) & "") = 0, "-", arrRecords1(35, i) & "")  ' Z向面缩3
        xlSheet.Cells(II + 21, 20).Value = IIf(Val(arrRecords1(36, i) & "") = 0, "-", arrRecords1(36, i) & "")  ' Z向面缩均值

        xlSheet.Cells(II + 21, 21).Value = "合格"    '冷弯
        ...
        xlSheet.Cells(II + 21, 27).Value = IIf(arrRecords1(37, i) = 0, "-", arrRecords1(37, i) & "")  ' 纤维率1
        xlSheet.Cells(II + 21, 28).Value = IIf(arrRecords1(38, i) = 0, "-", arrRecords1(38, i) & "")  ' 纤维率2
        xlSheet.Cells(II + 21, 29).Value = IIf(arrRecords1(39, i) = 0, "-", arrRecords1(39, i) & "")  ' 纤维率3
        xlSheet.Cells(II + 21, 30).Value = IIf(arrRecords1(40, i) = 0, "-", arrRecords1(40, i) & "")  ' 纤维率均值
        
        xlSheet.Cells(II + 22, 22).Value = IIf(arrRecords1(41, i) = 0, "-", arrRecords1(41, i) & "")  ' IMPACT_TMP 第二个冲击，利用追加冲击栏位
        ...
        xlSheet.Cells(II + 22, 27).Value = IIf(arrRecords1(46, i) = 0, "-", arrRecords1(46, i) & "")  ' 纤维率1
        xlSheet.Cells(II + 22, 28).Value = IIf(arrRecords1(47, i) = 0, "-", arrRecords1(47, i) & "")  ' 纤维率2
        xlSheet.Cells(II + 22, 29).Value = IIf(arrRecords1(48, i) = 0, "-", arrRecords1(48, i) & "")  ' 纤维率3
        xlSheet.Cells(II + 22, 30).Value = IIf(arrRecords1(49, i) = 0, "-", arrRecords1(49, i) & "")  ' 纤维率均值
        If arrRecords1(52, i) = "Y" Then
        xlSheet.Cells(II + 21, 31).Value = "合格"    '断口
        End If
        xlSheet.Cells(II + 21, 32).Value = "合格"    '探伤
         II = II + 2
...
'---- 其他标准 --------------------------------------------------------------------------------------------------------------
    Else
      ...
      xlSheet.Range("P38").Value = arrRecords1(31, 0) '冲击尺寸
      xlSheet.Range("C36").Value = RowCNT / 2 & "块"  '总计块数
     ...
     Bend_str = "弯曲角度=" + Str(arrRecords1(51, 0)) + "°" + "；弯心直径=" + Str(arrRecords1(50, 0)) + "a；试样宽度≥7a"
      ...
      '成分只取单数行记录
        ...
        xlSheet.Cells(i + 24, 18).Value = "合格"
        ...
        xlSheet.Cells(i + 24, 28).Value = IIf(arrRecords1(30, i) = 0, "-", arrRecords1(30, i) & "")  ' IMPACT_RST_AVE
                If arrRecords1(52, i) = "Y" Then
        xlSheet.Cells(i + 24, 30).Value = "合格"   '断口
        End If
    '    xlSheet.Cells(i + 24, 30).Value = "合格"
