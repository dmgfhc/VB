...
Begin VB.Form AAA1140C 
   ...
   Begin VB.ComboBox txt_prod_cd 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "AAA1140C.frx":0000
      Left            =   4245
      List            =   "AAA1140C.frx":0002
      TabIndex        =   2
      Tag             =   "产品代码"
      Top             =   180
      Width           =   870
   ...
   Begin VB.ComboBox cbo_plt 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "AAA1140C.frx":0004
      Left            =   1455
      List            =   "AAA1140C.frx":0006
      TabIndex        =   1
      Tag             =   "工厂"
      Top             =   180
      Width           =   870
   ...
   Begin FPSpread.vaSpread ss1 
      ...
      SpreadDesigner  =   "AAA1140C.frx":0008
   End
...
'-- Program Name      SLAB WIDTH STANDARD
'-- Program ID        AAA1140C
'-- Designer          GUOLI
'-- Coder             GUOLI
'-- Date              2009.6.6
...
'---------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   Call Gp_Sp_Collection(ss1, 14, " ", " ", " ", "i", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1)
    ...
    Sc1.Add Item:="AAA1140C.P_SMODIFY", Key:="P-M"
    Sc1.Add Item:="AAA1140C.P_SREFER", Key:="P-R"
    Sc1.Add Item:="AAA1140C.P_SONEROW", Key:="P-O"
...
Private Sub Form_Load()
    ...
    txt_prod_cd.AddItem "PP"
    txt_prod_cd.AddItem "HC"
    txt_prod_cd.AddItem "SL"
...
Dim sProdCd, sThkGrp, sWidGrp, sSlabWidGrp As String
    ss1.Row = Row
    ss1.Col = 2
    sProdCd = ss1.Text
...
If Col = 3 Then
    ss1.Col = Col
    sThkGrp = ss1.Text
    sQuery = "SELECT FR_THK || '~' || TO_THK FROM bp_thick_grp WHERE PROD_CD = '" + sProdCd + "' and THK_CD = '" + sThkGrp + "'  "
ElseIf Col = 5 Then
    ss1.Col = Col
    sWidGrp = ss1.Text
    sQuery = "SELECT FR_WID || '~' || TO_WID FROM BP_WIDTH_GRP WHERE PROD_CD = '" + sProdCd + "' and WID_CD = '" + sWidGrp + "'  "
ElseIf Col = 7 Then
    ss1.Col = Col
    sSlabWidGrp = ss1.Text
    sQuery = "SELECT FR_WID || '~' || TO_WID FROM BP_WIDTH_GRP WHERE PROD_CD = 'SL' and WID_CD = '" + sSlabWidGrp + "'  "
End If

If Col = 3 Or Col = 5 Or Col = 7 Then
    ss1.Col = Col + 1
...
Private Sub ss1_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    If ss1.ActiveCol = 3 Or ss1.ActiveCol = 5 Or ss1.ActiveCol = 7 Then
        ...
        If KeyCode = vbKeyF4 Then
           ...
           If ss1.ActiveCol = 3 Then
              Call Pf_Common_DD(M_CN1, KeyCode, sPROD_CD, "T")
           ElseIf ss1.ActiveCol = 5 Then
              Call Pf_Common_DD(M_CN1, KeyCode, sPROD_CD, "W")
           ElseIf ss1.ActiveCol = 7 Then
              Call Pf_Common_DD(M_CN1, KeyCode, "SL", "W")
           End If
...
Private Function Pf_Common_DD(Conn As ADODB.Connection, KeyCode As Integer, Prod_cd As String, TW As String) As Boolean
    ...
    If TW = "W" Then
        DD.sQuery = "SELECT WID_CD ""宽度组代码"", FR_WID || '~' || TO_WID ""宽度组范围"" FROM BP_WIDTH_GRP WHERE PROD_CD = '" + Prod_cd + "' ORDER BY WID_CD "
    ElseIf TW = "T" Then
        DD.sQuery = "SELECT THK_CD ""厚度组代码"", FR_THK || '~' || TO_THK ""厚度组范围"" FROM BP_THICK_GRP WHERE PROD_CD = '" + Prod_cd + "' ORDER BY THK_CD "
    End If
