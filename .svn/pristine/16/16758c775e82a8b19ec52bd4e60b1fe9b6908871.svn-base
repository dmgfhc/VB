...
Begin VB.Form CEG1040C 
   Caption         =   "加热炉均衡查询/调整_CEG1040C"
   ClientHeight    =   9225
   ClientLeft      =   75
   ClientTop       =   1995
   ClientWidth     =   15300
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin FPSpread.vaSpread ss1 
         ...
         RowHeaderDisplay=   0
         SpreadDesigner  =   "CEG1040C.frx":0052
      End
      Begin SSSplitter.SSSplitter SSSplitter2 
         ...
         Begin FPSpread.vaSpread ss2 
            ...
            MaxRows         =   2
            RetainSelBlock  =   0   'False
            SpreadDesigner  =   "CEG1040C.frx":0499
         End
      End
   End
   Begin InDate.ULabel ULabel3 
...
'-- Sub_System Name
'-- Program Name      MONTHLY SLAB USE PLAN FUR LINE CHANGE
'-- Program ID        CEG1040C
...
Private Sub Form_Define()
    ...
    sc2.Add Item:="CEG1040C.P_REFER2", Key:="P-R"
    sc2.Add Item:="CEG1040C.P_ONEROW2", Key:="P-O"
    sc2.Add Item:="CEG1040C.P_MODIFY2", Key:="P-M"
...
Public Sub Form_Ref()
    ...
    sQuery1 = sQuery1 + " WHERE  SLAB_FL <> 'A' "
    sQuery1 = sQuery1 + " ORDER  BY SLAB_THK ASC "
    
    'Data Display
    sQuery2 = " {call CEG1040C.P_DATA ()} "
    ...
    If sMessg = "OK" Then
        ...
        If sMessg = "OK" Then
            ...
            If Sp_Data_Refer1(ss1, sQuery2) Then     'SLAB Data Display
                ...
                Active_Col = 0
                Call Gp_Sp_ReadOnlySet(Sc1.Item("Spread"))
                sdb_slab_cnt1.Value = Gf_FloatFind(M_CN1, "SELECT SUM(ORD_CNF_SLAB_CNT) FROM EP_MON_REQ_SLAB WHERE FUR_LINE = '1' AND SLAB_FL <> 'A' ")
                sdb_slab_cnt2.Value = Gf_FloatFind(M_CN1, "SELECT SUM(ORD_CNF_SLAB_CNT) FROM EP_MON_REQ_SLAB WHERE FUR_LINE = '2' AND SLAB_FL <> 'A' ")
                sdb_slab_cnt3.Value = Gf_FloatFind(M_CN1, "SELECT SUM(ORD_CNF_SLAB_CNT) FROM EP_MON_REQ_SLAB WHERE FUR_LINE_AVA = '*' AND SLAB_FL <> 'A' ")
                sdb_slab_wgt1.Value = Gf_FloatFind(M_CN1, "SELECT SUM(SLAB_TOT_WGT) FROM EP_MON_REQ_SLAB WHERE FUR_LINE = '1' AND SLAB_FL <> 'A' ")
                sdb_slab_wgt2.Value = Gf_FloatFind(M_CN1, "SELECT SUM(SLAB_TOT_WGT) FROM EP_MON_REQ_SLAB WHERE FUR_LINE = '2' AND SLAB_FL <> 'A' ")
                sdb_slab_wgt3.Value = Gf_FloatFind(M_CN1, "SELECT SUM(SLAB_TOT_WGT) FROM EP_MON_REQ_SLAB WHERE FUR_LINE_AVA = '*' AND SLAB_FL <> 'A' ")
                sdb_slab_time1.Value = Gf_FloatFind(M_CN1, "SELECT SUM(FUR_TME)/60 FROM EP_MON_REQ_SLAB WHERE FUR_LINE = '1' AND SLAB_FL <> 'A' ")
                sdb_slab_time2.Value = Gf_FloatFind(M_CN1, "SELECT SUM(FUR_TME2)/60 FROM EP_MON_REQ_SLAB WHERE FUR_LINE = '2' AND SLAB_FL <> 'A' ")
                sdb_slab_time3.Value = Gf_FloatFind(M_CN1, "SELECT SUM(DECODE(FUR_LINE,'1',FUR_TME,'2',FUR_TME2))/60 FROM EP_MON_REQ_SLAB WHERE FUR_LINE_AVA = '*' AND SLAB_FL <> 'A' ")
                sdb_mill_time.Value = Gf_FloatFind(M_CN1, "SELECT SUM(MILL_TME)/60 FROM EP_MON_REQ_SLAB WHERE SLAB_FL <> 'A'")
            End If
...
Private Sub ss1_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    txt_fur_line.Text = Str(Col Mod 4)
    ss1.Row = 0
    ss1.Col = Col
