...
Const SS1_HTM_METH1 = 30  '是否热处理标记蓝色 2013-05-28  by  LiQian
...
Const SS1_URGNT_FL = 54   '紧急订单绿色标记 2012-08-16  by  LiQian
...
Call Gp_Ms_Collection(TXT_TRNS_CMPY_CD_TO, "p", " ", " ", " ", " ", " ", " ", pControl, nControl, mControl, iControl, rControl, aControl, lControl)
  'ADDED BY zhangjinbo for add 原始坯料钢种查询条件 at 20110121
    ...
    Call Gp_Sp_Collection(ss1, 54, " ", " ", " ", " ", " ", " ", pColumn, nColumn, mColumn, iColumn, aColumn, lColumn)    '紧急订单绿色标记 2012-08-16  by  LiQian
    Call Gp_Sp_Collection(ss1, 55, " ", " ", " ", " ", " ", " ", pColumn, nColumn, mColumn, iColumn, aColumn, lColumn)    '是否翻板
    Call Gp_Sp_Collection(ss1, 56, " ", " ", " ", " ", " ", " ", pColumn, nColumn, mColumn, iColumn, aColumn, lColumn)    '是否经RH 2013-05-28  by  LiQian
...
Public Sub Form_Ref()
    ...
    If TXT_TRNS_CMPY_CD.Text <> "" Or TXT_TRNS_CMPY_CD_TO.Text <> "" Then
            If SDT_PROD_DATE_FROM.Text = SDT_PROD_DATE_TO.Text Then
                Else: Call Gp_MsgBoxDisplay("切割时间必须为同一天", "W")
                Exit Sub
            End If
    End If
...
'               (Len(.Text) = 8 Or Len(.Text) = 10 Or Len(.Text) = 12 Or Len(.Text) = 14) Then
       ...
       ss1.Col = SS1_LEN_TOL:              xlApp.Range("O1").Value = ss1.Text
        ...
        xlApp.Range("E2").Value = "厚度公差"
        xlApp.Range("F2").Value = "厚度"
        xlApp.Range("G2").Value = "宽度"
        xlApp.Range("H2").Value = "长度"
        xlApp.Range("I2").Value = "重量"
        xlApp.Range("J2").Value = "切边"
        xlApp.Range("K2").Value = "定尺"
