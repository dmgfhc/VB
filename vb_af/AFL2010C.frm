...
Begin VB.Form AFL2010C 
   Caption         =   "板坯库库存修改及查询界面_AFL2010C"
   ClientHeight    =   7725
   ClientLeft      =   480
   ClientTop       =   3045
   ClientWidth     =   12435
   ...
   Begin VB.ComboBox cbo_ccm_line 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "AFL2010C.frx":0000
      Left            =   2220
      List            =   "AFL2010C.frx":0002
      TabIndex        =   6
      Tag             =   "连铸机号"
      Top             =   135
      Visible         =   0   'False
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin FPSpread.vaSpread ss1 
         ...
         SpreadDesigner  =   "AFL2010C.frx":0056
      End
      Begin FPSpread.vaSpread ss2 
         ...
         Protect         =   0   'False
         RetainSelBlock  =   0   'False
         SpreadDesigner  =   "AFL2010C.frx":0859
      End
   End
...
Const SS1_BED_SEQ = 1               'BED_SEQ
Const SS1_SLAB_NO = 2               'SLAB_NO
Const SS1_YARD_ADDR = 4             'YARD_ADDR
Const SS1_STLGRD = 5                'STLGRD
Const SS1_WID = 7                   'WID
Const SS1_LEN = 8                   'LEN
Const SS1_EMP_CD = 15               'INS_EMP_CD
Const SS1_FROM_ADDR = 16            'FROM_ADDR
Const SS2_BED_SEQ = 1               'BED_SEQ
Const SS2_SLAB_NO = 2               'SLAB_NO
Const SS2_YARD_ADDR = 4             'YARD_ADDR
Const SS2_STLGRD = 5                'STLGRD
Const SS2_WID = 7                   'WID
Const SS2_LEN = 8                   'LEN
Const SS2_EMP_CD = 15               'INS_EMP_CD
Const SS2_FROM_ADDR = 16            'FROM_ADDR
...
Private Sub Form_Define()
   ...
   Call Gp_Sp_Collection(ss2, 16, " ", " ", " ", "i", " ", "l", pColumn2, nColumn2, mColumn2, iColumn2, aColumn2, lColumn2)
    ...
    sc1.Add Item:="AFL2010C.P_MODIFY1", Key:="P-M"
    sc1.Add Item:="AFL2010C.P_REFER1", Key:="P-R"
    ...
    sc2.Add Item:="AFL2010C.P_MODIFY1", Key:="P-M"
    sc2.Add Item:="AFL2010C.P_REFER2", Key:="P-R"
    ...
    Call Gp_Sp_ColHidden(ss1, SS1_FROM_ADDR, True)
    Call Gp_Sp_ColHidden(ss2, SS2_FROM_ADDR, True)
...
Private Sub Form_Activate()
    ...
    txt_o_f_addr_nm.Text = Gf_ComnNameFind(M_CN1, "F0009", Trim(txt_f_addr.Text), 2)
...
Public Sub Form_Ref()
    ...
    If Trim(txt_f_addr) <> "" And Trim(txt_f_addr) <> "S0A0101" And Trim(txt_f_addr) <> "S0Q0101" Then
        sQuery = "SELECT * FROM FP_STDYARD WHERE LOCATION = '" + txt_f_addr + "' AND YARD_KND = '" + CBO_CUR_INV + "'"
    ...
    If Trim(txt_t_addr) <> "" And Trim(txt_t_addr) <> "S0A0101" And Trim(txt_t_addr) <> "S0Q0101" Then
        sQuery = "SELECT * FROM FP_STDYARD WHERE LOCATION = '" + txt_t_addr + "' AND YARD_KND = '" + CBO_CUR_INV + "'"
        If Gf_FloatFind(M_CN1, sQuery) = 0 Then
    ...
    If txt_f_addr = "S0A0101" Or txt_f_addr = "S0Q0101" Then
       Call Gp_Sp_ColHidden(ss1, SS1_BED_SEQ, True)
    Else
       Call Gp_Sp_ColHidden(ss1, SS1_BED_SEQ, False)
    End If
    
    If txt_t_addr = "S0A0101" Or txt_t_addr = "S0Q0101" Then
       Call Gp_Sp_ColHidden(ss2, SS2_BED_SEQ, True)
    Else
       Call Gp_Sp_ColHidden(ss2, SS2_BED_SEQ, False)
    End If
    ...
    If sMesg = "OK" Then
        ...
        If sMesg = "OK" Then
            ...
            If Gf_Sp_Refer(M_CN1, sc1, Mc1, Nothing, Nothing, False) Then
                ...
                With ss1
                     For iRow = MaxCnt To 1 Step -1
                         .Row = iRow
                         For iCol = SS1_BED_SEQ To .MaxCols
                             .Col = iCol
                             sTemp = .Text
                             .Text = ""
                    ...
                    If (sFromLoc = "S0A0101" Or sFromLoc = "S0Q0101" Or Trim(sFromLoc) = "") And txt_slab_no <> "" Then
                       .Col = SS1_SLAB_NO                                          '2
                       For iRow = Max_Rows - MaxCnt + 1 To Max_Rows
                           ...
                           If .Text = txt_slab_no Then
                              ...
                              iMoveCnt = 1
                              txt_slab_cnt = 1
                              For iCol = SS1_BED_SEQ To SS1_FROM_ADDR              '1，16
                                  .Col = iCol
                                  .BackColor = &HFF
                              Next iCol
            ...
            Else
                ...
                sc1.Item("Spread").Col = SS1_BED_SEQ    '1
            ...
            If Gf_Sp_Refer(M_CN1, sc2, Mc1, Nothing, Nothing, False) Then
                ...
                sc2.Item("Spread").Col = SS2_BED_SEQ                                '1
                sc2.Item("Spread").Action = ActionActiveCell
                
                With ss2
                     For iRow = MaxCnt To 1 Step -1
                         .Row = iRow
                         For iCol = SS2_BED_SEQ To .MaxCols
                             .Col = iCol
                             sTemp = .Text
                             .Text = ""
                             .Row = .Row + Max_Rows - MaxCnt
                    ...
                    If txt_slab_no <> "" Then
                         sQuery = "SELECT * FROM FP_SLABYARD WHERE SLAB_NO = '" + txt_slab_no + "' AND YARD_KND = '" + CBO_CUR_INV + "'"
                         If Gf_FloatFind(M_CN1, sQuery) = 0 Then
                            .Row = Max_Rows - MaxCnt
                            .Col = 0
                            .Text = "Input"
                            ...
                            .Col = SS2_YARD_ADDR                                    '4
                            .Text = sToLoc
                            .Col = SS2_FROM_ADDR                                    '16
                            .Text = sFromLoc
                            ssc_can.Enabled = True
                        
                             For iCol = SS2_BED_SEQ To SS2_EMP_CD                   '1,15
                                .Col = iCol
                                .BackColor = &HFF
                             Next iCol
                         ...
                         Else
                            ...
                            If sFromLoc = "S0A0101" Or sFromLoc = "S0Q0101" Then
            ...
            Else
                ...
                If txt_slab_no <> "" And (sFromLoc = "S0A0101" Or sFromLoc = "S0Q0101" Or Trim(sFromLoc) = "") Then
                   sQuery = "SELECT * FROM FP_SLABYARD WHERE SLAB_NO = " + txt_slab_no + " AND YARD_KND = '" + CBO_CUR_INV + "'"
                   If Gf_FloatFind(M_CN1, sQuery) = 0 Then
...
'     If iToStaRow = 0 And AFL2040C.Active_CForm <> "" Then
    If AFL2040C.Active_CForm <> "" Then
       ...
       Next iCnt
        ...
        sQuery = "SELECT MAX_CNT FROM FP_STDYARD WHERE LOCATION ='" + sToLoc + "' AND YARD_KND = '" + CBO_CUR_INV + "'"
        ssc_move.Enabled = False
        'If ss2.Text <> "" Then
...
Private Sub opt_Right_Left_DblClick(VALUE As Integer)
    ...
    If txt_f_addr.Text <> "" Then
       ...
       If mResult = vbYes Then
           If Gp_LOC_Exec(Trim(txt_f_addr.Text), Trim(CBO_CUR_INV.Text)) = "" Then
...
Private Sub ss1_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    With ss1
       ...
       Next i
        ...
        If (txt_f_addr <> "S0A0101") And sFlag <> "Y" Then
        ...
        ElseIf txt_f_addr = "S0A0101" Then
...
Private Sub ss2_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    With ss2
       ...
       Next i
        ...
        If (txt_t_addr <> "S0A0101") And sFlag <> "Y" Then
            For iCnt = Row To 1 Step -1
                .Col = SS2_SLAB_NO      '2
                .Row = iCnt
                If .Text <> "" Then
...
Private Sub ssc_can_Process(oSpr1 As vaSpread, oSpr2 As vaSpread)
    ...
    With oSpr1
      ...
      For iCnt = iFromRow - iMoveCnt + 1 To iFromRow Step 1
         ...
         .Col = 0
         .Text = ""
         For i = SS1_BED_SEQ To SS1_YARD_ADDR        '1,4
          .Col = i
          .BackColor = &HC0FFFF
         Next i
         
         For i = SS1_STLGRD To .MaxCols               '5
          .Col = i
          .BackColor = &HFFFFFF
         Next i
      Next
    ...
    With oSpr2
      ...
      For iCnt = iToStaRow To iToStaRow - iMoveCnt + 1 Step -1
         ...
         .Col = 0
         .Text = ""
         For i = SS2_BED_SEQ To SS2_YARD_ADDR        '1,4
          .Col = i
          .BackColor = &HC0FFFF
          .Text = ""
         Next i
         
         For i = SS2_STLGRD To .MaxCols               '5
          .Col = i
          .BackColor = &HFFFFFF
         Next
...
Private Sub ssc_move_Process(oSpr1 As vaSpread, oSpr2 As vaSpread)
    ...
    With oSpr2
        ...
        For iCnt = iToStaRow To iToStaRow - iMoveCnt + 1 Step -1
              ...
              For i = SS2_BED_SEQ To .MaxCols    '1
                .Col = i
                .BackColor = &HFF
              Next
              .Col = SS2_BED_SEQ       '1
        ...
        oSpr2.Col = SS2_SLAB_NO        '2
            ...
            If sToLoc <> "S0A0101" And sToLoc <> "S0Q0101" And To_Bedseq > Gf_FloatFind(M_CN1, sQuery) Then
               MsgBox "已超出目的垛位的存放能力！当前操作无法继续！", vbCritical, "系统提示信息"
               Call ssc_can_Click
               Exit Sub
            End If
...
Dim sQuery As String
    If Len(txt_f_addr) = 7 And Trim(txt_f_addr) <> "S0A0101" And Trim(txt_f_addr) <> "S0Q0101" Then
       sQuery = "SELECT * FROM FP_STDYARD WHERE LOCATION = '" + txt_f_addr + "' AND YARD_KND = '" + CBO_CUR_INV + "'"
       If Gf_FloatFind(M_CN1, sQuery) = 0 Then
...
Private Sub txt_f_addr_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyF4 Then
        ...
        If CBO_CUR_INV.Text = "00" Then
           DD.sKey = "F0009"
        ElseIf CBO_CUR_INV.Text = "HB" Then
...
Dim sQuery As String
    If Len(txt_t_addr) = 7 And Trim(txt_t_addr) <> "S0A0101" And Trim(txt_t_addr) <> "S0Q0101" Then
       sQuery = "SELECT * FROM FP_STDYARD WHERE LOCATION = '" + txt_t_addr + "' AND YARD_KND  = '" + CBO_CUR_INV + "'"
       If Gf_FloatFind(M_CN1, sQuery) = 0 Then
   ...
   If txt_f_addr <> "S0A0101" And txt_f_addr <> "S0Q0101" And Len(txt_t_addr) = 7 Then
      txt_slab_no = ""
   End If
End Sub
...
Public Function Gp_LOC_Exec(sAddr As String, CBOTEXT As String) As String
...
'    sQuery = "{call AFL2010C.P_LOC_TUN ('" + "00" + "','" + sAddr + "',?)}"
   sQuery = "{call AFL2010C.P_LOC_TUN ('" + CBOTEXT + "','" + sAddr + "',?)}"
...
Private Sub ULabel6_DblClick()
    ...
    If txt_t_addr.Text <> "" Then
       ...
       mResult = MsgBox(sMsg, vbYesNo, "系统提示信息")
       If mResult = vbYes Then
           If Gp_LOC_Exec(Trim(txt_t_addr.Text), Trim(CBO_CUR_INV.Text)) = "" Then
              MsgBox ("垛位调整完毕 ！"), vbInformation, "系统提示信息"
              Call Form_Ref
           Else
              MsgBox (" 垛位调整失败！"), vbCritical, "系统提示信息"
