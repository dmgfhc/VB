...
Begin VB.Form AQX0010C 
   Caption         =   "订单用途代码选择_AQX0010C"
   ClientHeight    =   7665
   ClientLeft      =   795
   ClientTop       =   2145
   ClientWidth     =   8835
   ...
   Begin Threed.SSPanel pnl_result 
      ...
      Begin FPSpread.vaSpread ssResult 
         ...
         SpreadDesigner  =   "AQX0010C.frx":0000
      End
   End
   Begin Threed.SSPanel pnl_condition 
      ...
      Begin FPSpread.vaSpread ssWhere 
         ...
         ScrollBarExtMode=   -1  'True
         ScrollBars      =   1
         SpreadDesigner  =   "AQX0010C.frx":028C
         UserResize      =   1
      End
   End
...
Private Sub cmd_Selection_Click()
    ...
    If ssResult.ActiveRow > 0 Then
        ...
        AQC0360C.txt_enuse_chg_name.Text = ssResult.Text
...
' .Clip = "enduse_cd" + Chr(9) + "enduse_name" + Chr(9) + "prod_knd" + Chr(9) + "stdspec" + Chr(9) + "thk_min" + Chr(9) + "thk_max" + Chr(9)
 .Clip = "订单用途代码" + Chr(9) + "订单用途" + Chr(9) + "产品品种" + Chr(9) + "标准" + Chr(9) + "最小厚度" + Chr(9) + "最大厚度" + Chr(9) + "使用标记" + Chr(9)
End With
...
End With
   ...
   sQuery = "SELECT A.enduse_cd,B.ENDUSE_NAME,A.prod_knd,A.stdspec,A.thk_min,A.thk_max,A.ENDUSE_FL FROM qp_std_usage A, qp_ord_usage B "
   sQuery = sQuery + " WHERE A.ENDUSE_CD=B.ENDUSE_CD and a.prod_knd=b.prod_knd  "
   sQuery = sQuery + " AND A.PROD_KND = SUBSTR( '" & Trim(AQC0360C.txt_PROD_CD.Text) & "', 1, 1) AND A.ENDUSE_FL <>'H'  "
