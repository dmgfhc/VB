...
Begin VB.Form ACB6050C 
   Caption         =   "板坯卸车实绩录入_ACB6050C"
   ClientHeight    =   9225
   ClientLeft      =   315
   ClientTop       =   1380
   ClientWidth     =   15030
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin FPSpread.vaSpread ss2 
         ...
         SpreadDesigner  =   "ACB6050C.frx":0052
      End
      Begin FPSpread.vaSpread ss1 
         ...
         ScrollBarExtMode=   -1  'True
         SpreadDesigner  =   "ACB6050C.frx":0786
      End
   End
   Begin VB.Line Line1 
...
Const ss1_slab_no = 3               ' 物料号
Const ss1_grade = 12                ' 钢种说明
Const ss1_is_jiepi = 13                  ' 是否接坯
Const SS1_PLT = 15                  ' 生产工厂
Const ss1_cut_date = 16             ' 生产日期
Const ss1_size_sta = 7              ' 厚度、宽度、长度、 重量 开始
Const ss1_size_end = 10               ' 厚度、宽度、长度、 重量 结束

Const ss1_date = 24                 ' 到达日期
Const ss1_emp_no = 25               ' 录入人
...
Private Sub Form_Define()
    ...
    Call Gp_Sp_Collection(ss1, 4, " ", " ", " ", " ", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1)   '坯料类别
   ...
   Call Gp_Sp_Collection(ss1, 28, " ", " ", " ", " ", " ", "l", pColumn1, nColumn1, mColumn1, iColumn1, aColumn1, lColumn1)
    ...
    sc1.Add Item:="ACB6050C.P_REFER", Key:="P-R"
    sc1.Add Item:="ACB6050C.P_ONEROW", Key:="P-O"
    sc1.Add Item:="ACB6050C.P_MODIFY", Key:="P-M"
...
Private Sub cmd_input_Click()
    ...
    With ss1
        For iDx = 1 To .MaxRows
            ...
            .Col = ss1_date:       .Text = Trim(txt_input_date.Text)
            .Col = ss1_emp_no:     .Text = sUserID
            .Col = 0:      .Text = "Update"
        Next iDx
    End With
...
'  ADD BY YIDUJUN AT 20100526142100
    ...
    If ss1.Row <> 0 Then
        If ss1.Col = 2 Then
            ...
            S_NO = ss1.Text
            sQuery1 = "SELECT COUNT(*) FROM FP_SLAB WHERE SLAB_NO IN (SELECT MAT_NO FROM CP_MOVE_SLT WHERE MV_LST_NO = '" + S_NO + "' ) AND (LOC = '00' OR LOC = 'S0X0101' OR LOC = 'S0Y0101')  "
            sQuery2 = "SELECT COUNT(*) FROM FP_SLAB WHERE SLAB_NO IN (SELECT MAT_NO FROM CP_MOVE_SLT WHERE MV_LST_NO = '" + S_NO + "' ) AND CUR_INV <> 'ZZ'"
            sQuery3 = "SELECT COUNT(*) FROM FP_SLAB WHERE SLAB_NO IN (SELECT MAT_NO FROM CP_MOVE_SLT WHERE MV_LST_NO = '" + S_NO + "' ) AND REC_STS <> '3'"
            
            If Gf_FloatFind(M_CN1, sQuery1) = 0 Then
                Call Gp_MsgBoxDisplay("板坯已经入库，需变更垛位请到板坯库库存修改及查询界面_ACB6060C操作！")
                    Exit Sub
            ElseIf Gf_FloatFind(M_CN1, sQuery2) = 0 Then
                Call Gp_MsgBoxDisplay("板坯未卸车，请先做卸车！")
                    Exit Sub
            ElseIf Gf_FloatFind(M_CN1, sQuery3) = 0 Then
                Call Gp_MsgBoxDisplay("板坯已分切耗用！")
                    Exit Sub
            Else
                Unload ACB6060C
                Load ACB6060C
                ACB6060C.Show
                ss1.Col = 2
                ACB6060C.txt_mv_lst_no = ss1.Text
                ACB6060C.cbo_inv = Mid(ss1.Text, 4, 2)
                If Mid(ss1.Text, 4, 2) = "XK" Then
                     ACB6060C.txt_f_addr = "S0X0101"
                ElseIf Mid(ss1.Text, 4, 2) = "52" Then
                     ACB6060C.txt_f_addr = "S0Y0101"
                End If
                Call ACB6060C.Form_Ref
            End If
        End If
    End If
...
Private Sub ss2_DblClick(ByVal Col As Long, ByVal Row As Long)
    ...
    With ss1
        For iRow = 1 To .MaxRows
                .Row = iRow:  .Col = 6
                If .Text = "S0X0101" Or .Text = "S0Y0101" Or .Text = "00" Then
...
'    Next I
  ...
  Clipboard.Clear
    ss1.SetSelection ss1_slab_no, 1, ss1_slab_no, ss1.MaxRows ' 物料号
'    ss1.SetSelection a, b, c,d   用法：  ss1  a列 到b列的数据 塞到 b 行 到d行的数据中  对应到 excel的 A5中
    ss1.ClipboardCopy
    xlApp.Range("A5").Select
    xlApp.ActiveSheet.Paste
    
    Clipboard.Clear
    ss1.SetSelection ss1_grade, 1, ss1_grade, ss1.MaxRows ' 钢种说明
    ss1.ClipboardCopy
    xlApp.Range("B5").Select
    xlApp.ActiveSheet.Paste
    
    Clipboard.Clear
    ss1.SetSelection ss1_is_jiepi, 1, ss1_is_jiepi, ss1.MaxRows ' 是否接坯
    ss1.ClipboardCopy
    xlApp.Range("C5").Select
    xlApp.ActiveSheet.Paste
    
    Clipboard.Clear
    ss1.SetSelection SS1_PLT, 1, SS1_PLT, ss1.MaxRows ' 生产工厂
    ss1.ClipboardCopy
    xlApp.Range("D5").Select
    xlApp.ActiveSheet.Paste
    
    Clipboard.Clear
    ss1.SetSelection ss1_cut_date, 1, ss1_cut_date, ss1.MaxRows ' 生产日期
    ss1.ClipboardCopy
    xlApp.Range("E5").Select
    xlApp.ActiveSheet.Paste
    
    Clipboard.Clear
    ss1.SetSelection ss1_size_sta, 1, ss1_size_end, ss1.MaxRows ' 厚度、宽度、长度、 重量
    ss1.ClipboardCopy
